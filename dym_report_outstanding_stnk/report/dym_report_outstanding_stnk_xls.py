...
from .dym_report_outstanding_stnk import dym_report_outstanding_stnk_print
from openerp.tools.translate import translate
import logging
_logger = logging.getLogger(__name__)
import string

_ir_translation_name = 'report.oustanding.stnk'

class dym_report_outstanding_stnk_print_xls(dym_report_outstanding_stnk_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_outstanding_stnk_print_xls, self).__init__(
            cr, uid, name, context=context)
        move_line_obj = self.pool.get('dym.penerimaan.stnk')
        self.context = context
        wl_overview = move_line_obj._report_xls_outstanding_stnk_fields(
            cr, uid, context)
...
class report_outstanding_stnk_xls(report_xls):

    def __init__(self, name, table, rml=False,
                 parser=False, header=True, store=False):
        super(report_outstanding_stnk_xls, self).__init__(
        ...
        self.col_specs_template_overview = {
            ...
            'trans_no': {
                'header': [1, 30, 'text', _render("_('Nomor Tagihan Birojasa')")],
                'lines': [1, 0, 'text', _render("p['trans_no']")],
                'totals': [1, 30, 'text', None]},
            'branch_name': {
            ...
            'invoice_tagihan_number': {
                'header': [1, 30, 'text', _render("_('No Faktur Tagihan Birojasa')")],
                'lines': [1, 0, 'text', _render("p['invoice_tagihan_number']")],
                'totals': [1, 30, 'text', None]},
            'invoice_tagihan_date': {
                'header': [1, 30, 'text', _render("_('Tgl Faktur Tagihan Birojasa')")],
                'lines': [1, 0, 'text', _render("p['invoice_tagihan_date']")],
                'totals': [1, 30, 'text', None]},
            'invoice_dso_number': {
                'header': [1, 30, 'text', _render("_('No Faktur Penjualan')")],
            ...
            'proses_stnk': {
                'header': [1, 25, 'text', _render("_('No Proses STNK')")],
                'lines': [1, 0, 'text', _render("p['proses_stnk']")],
                'totals': [1, 25, 'text', None]},
            'margin': {
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            report_name = ' '.join(
                [_('LAPORAN OUTSTANDING STNK Per Tanggal'), _(str(datetime.today().date())),
                 _p.report_info])
            c_specs_o = [
                ('report_name', 1, 0, 'text', report_name),
            ]
...
report_outstanding_stnk_xls('report.Laporan Outstanding STNK', 'dym.penerimaan.stnk', parser = dym_report_outstanding_stnk_print_xls)
