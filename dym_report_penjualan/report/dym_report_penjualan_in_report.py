...
class dym_report_penjualan_report(osv.osv):
    _name = "dym.report.penjualan.report"
    ...
    def init(self, cr):
        tools.sql.drop_view_if_exists(cr, 'dym_report_penjualan_report')
        cr.execute("""
            create or replace view dym_report_penjualan_report as (
                select min(dso.id) as id,  
            dso.branch_id as branch_id,  
            da_db.area_id as area_id,  
            dso.name,  
            ...
            left join (select   da.id as area_id, da.code as area_code, db.name as branch_name, db.id as branch_id 
            from    dym_branch db
            left join dym_area_cabang_rel dacr on db.id = dacr.branch_id
            left join dym_area da on da.id = dacr.area_id
            where   da.code in (
                    'Area Sulawesi, Bali & Jatim',
                    'Area RMR & Jakarta',
                    'Area Palembang, Babel, Lampung',
                    'Area Sumut, Jambi, Padang & Bengkulu',
                    'Area Kalbar, Kalsel, Kaltim',
                    'Area Jabar Barat',
                    'Area Jabar Tengah',
                    'Area Jabar Timur') and
                db.branch_status not in ('H23','HO')
            order by 1,2) da_db on da_db.branch_id = b.id
            WHERE  1=1 and dso.state in ('progress','done')
