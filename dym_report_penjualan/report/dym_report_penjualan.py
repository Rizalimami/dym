...
class dym_report_penjualan_print(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        if context is None:
            context = {}
        super(dym_report_penjualan_print, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        report_penjualan = {
            'type': '',
            'title': '',
            'title_short': title_short_prefix + ', ' + _('Laporan Penjualan')}
        ...
        query_penjualan = """
		    ...
		    case when dso.is_pic <> 't' then COALESCE(cust_stnk.name,'') else '' end as cust_name_stnk, 
            ...
            """group by dso.id, drsl.name, dsol.price_subtotal, dsol.price_bbn, b.branch_status, b.code, spk.name, dcp.name, b.name, md.default_code, finco.name, employee_koord.name_related,
             ...
             hk.name, pro.number, pro.name, sp.state, sp.name, cust.mobile, cust_stnk.mobile, dspl.tahun_pembuatan, dsol.finco_no_po, dsol.finco_tgl_po,employee_spv.name_related """ \
            "order by 6,13"
        ...
        reports = [report_penjualan]
        for report in reports:
            cr.execute(query_penjualan)
            # print query_penjualan
            query_penjualan
            ...
            if all_lines:
                ...
                p_map = map(
                    lambda x: {
                        ...
                        'cust_name_stnk': str(x['cust_name_stnk'].encode('ascii','ignore').decode('ascii')) if x['cust_name_stnk'] != None else '',
			...
			now = datetime.now()
                        ...
                        if invoice_ids:
                            ...
                            date_effective = moves.date
                            # date_effective = datetime.strptime(date_effective, '%Y-%m-%d')
                            if invoices.date_invoice:
                                date_inv = invoices.date_invoice
                            else:
                                date_inv = invoices.confirm_date
                                date_inv = datetime.strptime(date_inv, '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d')
        ...
        for x in reports[0]['dso_ids']:
            ...
            if x['tgl_lunas'] and x['invoice_date']:
                ...
                x['ar_days'] = abs((datetime.strptime(x['tgl_lunas'],'%d-%m-%Y') - datetime.strptime(x['invoice_date'],'%d-%m-%Y')).days) + 1
            elif not x['tgl_lunas'] and x['invoice_date']:
                x['tgl_lunas'] = None
                    ...
                    for z in y['lines']:
                        if z['lot_name'] != temp_str:
                            ...
                            test_tampung = {
                                ...
                                'cust_name_stnk' : temp['cust_name_stnk'],
                                'piutang' : z['piutang'],
        ...
        if not reports :
            ...
            'no_po': 'NO DATA FOUND',
            'faktur_pajak': 'NO DATA FOUND'}], 'title_short': 'Laporan Penjualan', 'type': '', 'title': ''}]
        ...
        objects=False
        super(dym_report_penjualan_print, self).set_context(
    ...
    def formatLang_zero2blank(self, value, digits=None, date=False, date_time=False, grouping=True, monetary=False, dp=False, currency_obj=False):
        if isinstance(value, (float, int)) and not value:
            ...
            return super(dym_report_penjualan_print, self).formatLang(value, digits, date, date_time, grouping, monetary, dp, currency_obj)

#register report
class wrapped_vat_declaration_print(orm.AbstractModel):
    _name = 'report.dym_report_penjualan.report_penjualan'
    _inherit = 'report.abstract_report'
    _template = 'dym_report_penjualan.report_penjualan'
    _wrapped_report_class = dym_report_penjualan_print
