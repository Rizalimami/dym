...
class dym_mutasi_stnk(osv.osv):
    _name = "dym.mutasi.stnk"
    ...
    _columns = {
        ...
        'mutasi_line': fields.one2many('dym.mutasi.stnk.line','mutasi_stnk_id',string="Table Mutasi STNK"), 
        'tgl_mutasi' : fields.date('Tanggal Mutasi'),
        'source_location_id' : fields.many2one('dym.lokasi.stnk',string="Source Location"),
        'destination_location_id' : fields.many2one('dym.lokasi.stnk',string="Destination Location"), 
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        mutasi_pool = self.pool.get('dym.mutasi.stnk.line')
        vals['name'] = self.pool.get('ir.sequence').get_per_branch(cr, uid, vals['branch_id'], 'MST', division=vals['division'])
        
        vals['tgl_mutasi'] = time.strftime('%Y-%m-%d'),
        del[vals['mutasi_line']]

        
        mutasi_id = super(dym_mutasi_stnk, self).create(cr, uid, vals, context=context) 
    ...
    def write(self,cr,uid,ids,vals,context=None):
        ...
        return super(dym_mutasi_stnk, self).write(cr, uid, ids, vals,context=context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(dym_mutasi_stnk, self).unlink(cr, uid, ids, context=context)
            
class dym_mutasi_stnk_line(osv.osv):
    _name = "dym.mutasi.stnk.line"
    _columns = {
                'name' : fields.many2one('stock.production.lot','No Engine',domain="[('lokasi_stnk_id','=',parent.source_location_id)]",change_default=True,),
                'mutasi_stnk_id' : fields.many2one('dym.mutasi.stnk','Mutasi STNK'),
