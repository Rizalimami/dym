...
class dym_mutasi_bpkb(osv.osv):
    _name = "dym.mutasi.bpkb"
    ...
    _columns = {
        ...
        'destination_branch_id': fields.many2one('dym.branch', string='Destination Branch', required=True),
        ...
        'mutasi_line': fields.one2many('dym.mutasi.bpkb.line','mutasi_bpkb_id',string="Table Mutasi BPKB"), 
        'tgl_mutasi' : fields.date('Tanggal Mutasi'),
        'source_location_id' : fields.many2one('dym.lokasi.bpkb',string="Source Location"),
        'destination_location_id' : fields.many2one('dym.lokasi.bpkb',string="Destination Location"),
    ...
    _defaults = {
      'state':'draft',
      'division' : 'Unit',
      'tgl_mutasi': fields.date.context_today,
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        mutasi_pool = self.pool.get('dym.mutasi.bpkb.line')
        vals['name'] = self.pool.get('ir.sequence').get_per_branch(cr, uid, vals['branch_id'], 'MBP', division=vals['division'])
        
        vals['tgl_mutasi'] = time.strftime('%Y-%m-%d'),
        del[vals['mutasi_line']]

        
        mutasi_id = super(dym_mutasi_bpkb, self).create(cr, uid, vals, context=context) 
    ...
    def write(self,cr,uid,ids,vals,context=None):
        ...
        return super(dym_mutasi_bpkb, self).write(cr, uid, ids, vals,context=context)
     
    def unlink(self, cr, uid, ids, context=None):
        for item in self.browse(cr, uid, ids, context=context):
            if item.state != 'draft':
                raise osv.except_osv(('Perhatian !'), ("Mutasi sudah di post ! tidak bisa didelete !"))
        return super(dym_mutasi_bpkb, self).unlink(cr, uid, ids, context=context)
            
class dym_mutasi_bpkb_line(osv.osv):
    _name = "dym.mutasi.bpkb.line"
    _columns = {
                'name' : fields.many2one('stock.production.lot','No Engine',domain="[('lokasi_bpkb_id','=',parent.source_location_id)]",change_default=True,),
                'mutasi_bpkb_id' : fields.many2one('dym.mutasi.bpkb','Mutasi BPKB'),
