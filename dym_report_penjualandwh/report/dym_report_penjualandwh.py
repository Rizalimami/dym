...
class dym_report_penjualandwh_print(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        if context is None:
            context = {}
        super(dym_report_penjualandwh_print, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        chosen_month = end_date[5:7]
        branch_ids = data['branch_ids']
        print "chosen_monthchosen_month", chosen_month
        title_short_prefix = ''
        ...
        report_penjualandwh = {
            'type': '',
            'title': '',
            'title_short': title_short_prefix + ', ' + _('Laporan DWH New')} 
        ...
        query_dwh = """
            select  dso.id as id_dso, 
                    ...
                    --employee_koord.tgl_masuk as lama_kerja,
                ...
                left join (select dso.name as dso_name, count(*) as qty_retur from dym_retur_jual drj left join dealer_sale_order dso on dso.id = drj.dso_id group by dso.name) rj on rj.dso_name = dso.name
                 """ \
        ...
        reports = [report_penjualandwh]
        for report in reports:
            cr.execute(query_dwh)
            print query_dwh
            ...
            if all_lines:
                ...
                for p in p_map:
                    if p['id_dso'] not in map(lambda x: x.get('id_dso', None), dso_ids):
                        ...
                        dsol = self.pool.get('dealer.sale.order.line').browse(cr, uid, p['id_dso'])
                        try:
                        ...
                        if invoice_ids:
                            ...
                            if payment_dates:
                                ...
                                payment_dates = None
                            if p['dso_amount'] - sum(payment_credits) == 0:
                            ...
                            if invoices.date_invoice:
                                ...
                                p.update({'invoice_date_year': datetime.strptime(invoices.date_invoice, '%Y-%m-%d').strftime('%Y')})
                                p.update({'invoice_date_date': datetime.strptime(invoices.date_invoice, '%Y-%m-%d').strftime('%d')})
                            else:
                                p.update({'invoice_date': datetime.strptime(invoices.confirm_date, '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d')})
                                p.update({'last_update_time': datetime.strptime(invoices.write_date, '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d')})
                                p.update({'invoice_date_year': datetime.strptime(invoices.confirm_date, '%Y-%m-%d %H:%M:%S').strftime('%Y')})
                                p.update({'invoice_date_date': datetime.strptime(invoices.confirm_date, '%Y-%m-%d %H:%M:%S').strftime('%d')})
                        ...
                        dso_ids.append(p)
                        p.update({'chosen_month':chosen_month})
        ...
        for x in reports[0]['dso_ids']:
            ...
            if x['tgl_lunas'] and x['invoice_date']:
                    ...
                    for z in y['lines']:
                        if z['lot_name'] != temp_str:
                            ...
                            test_tampung = {
                                ...
                                'invoice_date_year' : temp['invoice_date_year'],
                                'invoice_date_date' : temp['invoice_date_date'],
                                'chosen_month' : temp['chosen_month'],
                                'marketing' : temp['marketing'],
                                'kota_cabang' : temp['kota_cabang'],
                                'tanda_jadi': temp['tanda_jadi'],
                                'mkt_join_date': temp['mkt_join_date'],
                                'lama_kerja': temp['lama_kerja'],
                                'spv_nik': temp['spv_nik'],
                                'spv_name': temp['spv_name'],
                                'nama_pendek': temp['nama_pendek'],
                                ...
                                'ps_other' : z['ps_other'],
                                'jp_po' : z['jp_po'],
                                'discount_extern_ps' : z['discount_extern_ps'],
                                'discount_intern_ps' : z['discount_intern_ps'],
        ...
        if not reports :
            ...
            'invoice_date_year': 'NO DATA FOUND',
            'invoice_date_date': 'NO DATA FOUND',
            'chosen_month': 'NO DATA FOUND',
            'marketing' : 'NO DATA FOUND',
            'kota_cabang' : 'NO DATA FOUND',
            'tanda_jadi': 'NO DATA FOUND',
            ...
            'faktur_pajak': 'NO DATA FOUND'}], 'title_short': 'Laporan DWH New', 'type': '', 'title': ''}]
        ...
        objects = False
        super(dym_report_penjualandwh_print, self).set_context(
    ...
    def formatLang_zero2blank(self, value, digits=None, date=False, date_time=False, grouping=True, monetary=False, dp=False, currency_obj=False):
        if isinstance(value, (float, int)) and not value:
            ...
            return super(dym_report_penjualandwh_print, self).formatLang(value, digits, date, date_time, grouping, monetary, dp, currency_obj)

#register report
class wrapped_vat_declaration_print(orm.AbstractModel):
    _name = 'report.dym_report_penjualandwh.report_penjualandwh'
    _inherit = 'report.abstract_report'
    _template = 'dym_report_penjualandwh.report_penjualandwh'
    _wrapped_report_class = dym_report_penjualandwh_print
