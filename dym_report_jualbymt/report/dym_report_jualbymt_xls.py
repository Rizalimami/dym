...
from .dym_report_jualbymt import dym_report_jualbymt_print
from openerp.tools.translate import translate
import string

class dym_report_jualbymt_print_xls(dym_report_jualbymt_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_jualbymt_print_xls, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'datetime': datetime,
            'wanted_list_overview': self.pool.get('dealer.sale.order')._report_xls_jualbymt_fields(cr, uid, context),
            '_': self._,
        })

    def _(self, src):
        lang = self.context.get('lang', 'en_US') 
        return translate(self.cr, 'report.jualbymt', 'report', lang, src) or src

class report_jualbymt_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_jualbymt_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_overview = {
            ...
            'mt':{
                'header': [1, 30, 'text', _render("_('MT')")],
                'lines': [1, 0, 'text', _render("p['mt']")],
                'totals': [1, 30, 'number', None]},
            'leader':{
                'header': [1, 30, 'text', _render("_('Sales Coord')")],
                'lines': [1, 0, 'text', _render("p['leader']")],
                'totals': [1, 30, 'number', None]},
            'spv_name':{
            ...
            'motor':{
                ...
                'totals': [1, 25, 'number', None]},
            'is_cod':{
                'header': [1, 25, 'text', _render("_('Payment Type')")],
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ws_o = wb.add_sheet('Laporan Penjualan By MT')
            ...
            c_specs_o = [('report_name', 1, 0, 'text', 'Laporan Penjualan By MT')]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(ws_o, row_pos_o, row_data, row_style=cell_style)
...
report_jualbymt_xls('report.Laporan Penjualan By MT', 'dealer.sale.order', parser = dym_report_jualbymt_print_xls)
