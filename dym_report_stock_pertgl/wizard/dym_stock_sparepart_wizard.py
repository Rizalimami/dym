...
class report_stock_sparepart_pertgl_wizard(orm.TransientModel):
    _name = 'report.stock.sparepart.pertgl.wizard'
    _description = 'Report Stock Sarepart Per Tanggal Wizard'
 
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if not context: context = {}
        res = super(report_stock_sparepart_pertgl_wizard, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = {
        ...
        'branch_ids': fields.many2many('dym.branch', 'report_stock_sparepart_pertgl_invoice_rel', 'report_stock_sparepart_pertgl_wizard_id',
                                        'branch_id', 'Branch', copy=False),
        'product_ids': fields.many2many('product.product', 'report_stock_sparepart_pertgl_product_rel', 'report_stock_sparepart_pertgl_wizard_id',
                                        'product_id', 'Product', copy=False, ),
        'location_ids': fields.many2many('stock.location', 'report_stock_sparepart_pertgl_location_rel', 'report_stock_sparepart_pertgl_wizard_id',
    ...
    def create(self,cr,uid,vals,context=None):
        ...
        res = super(report_stock_sparepart_pertgl_wizard,self).create(cr,uid,vals,context=context)
        return res 
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        if context.get('xls_export'):
            return {'type': 'ir.actions.report.xml',
                    'report_name': 'dym.stock.sparepart.pertgl.report.xls',
                    'datas': data}
        else:
            context['landscape'] = True
            return self.pool['report'].get_action(
                cr, uid, [],
                'dym_report_stock_pertgl.report_stock_sparepart_pertgl',
                data=data, context=context)

    def xls_export_sparepart(self, cr, uid, ids, context=None):
