...
from .dym_report_stock_sparepart import dym_stock_sparepart_report_pertgl_print
...
class dym_stock_report_sparepart_print_pertgl_xls(dym_stock_sparepart_report_pertgl_print):

    def __init__(self, cr, uid, name, context):
        super(dym_stock_report_sparepart_print_pertgl_xls, self).__init__(
            cr, uid, name, context=context)
        quant_obj = self.pool.get('stock.quant')
        self.context = context
        wl_overview = quant_obj._report_xls_stock_sparepart_pertgl_fields(
            cr, uid, context)
...
class stock_report_sprepart_per_tgl_xls(report_xls):

    def __init__(self, name, table, rml=False,
                 parser=False, header=True, store=False):
        super(stock_report_sprepart_per_tgl_xls, self).__init__(
        ...
        self.col_specs_template_overview = {
            ...
            'product_name': {
                'header': [1, 22, 'text', _render("_('Kode Part dan Accessories')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['product_name'] or 'n/a'")],
                'totals': [1, 0, 'text', None]},
            'product_desc': {
                'header': [1, 22, 'text', _render("_('Deskripsi')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['product_desc'] or 'n/a'")],
                'totals': [1, 0, 'text', None]},       
            'ranking': {
                'header': [1, 15, 'text', _render("_('Ranking')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['ranking']")],
                'totals': [1, 0, 'text', None]},        
            'parent_category': {
                ...
                'totals': [1, 0, 'text', None]},   
            'categ_parent_name': {
                'header': [1, 22, 'text', _render("_('Group')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['categ_parent_name'] or 'n/a'")],
                'totals': [1, 0, 'text', None]}, 
            'categ_name': {
                'header': [1, 22, 'text', _render("_('Sub Group')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['categ_name'] or 'n/a'")],
                'totals': [1, 0, 'text', None]},  
            'total_stock': {
                'header': [1, 15, 'text', _render("_('Total Stock')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'number', _render("p['total_stock']")],
                'totals': [1, 0, 'text', None]},         
            'intransit': {
                'header': [1, 15, 'text', _render("_('Posisi Stock Intransit')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'number', _render("p['intransit']")],
                'totals': [1, 0, 'text', None]},         
            'rfs': {
                'header': [1, 15, 'text', _render("_('Posisi Stock Ready for Sale')"),None,self.rh_cell_style_center],
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            report_name = ' '.join(
                [_p.company.name, r['title'], _('Laporan Stock Sparepart ' + r['date']),
                 _p.report_info])
            c_specs_o = [
            ...
            ws_o.write(row_pos_o, 7, xlwt.Formula("SUM(H"+str(row_data_begin)+":H"+str(row_data_end)+")"),self.pd_cell_style_number_fill)
            ws_o.write(row_pos_o, 8, xlwt.Formula("SUM(I"+str(row_data_begin)+":I"+str(row_data_end)+")"),self.pd_cell_style_number_fill)
            ws_o.write(row_pos_o, 9, xlwt.Formula("SUM(J"+str(row_data_begin)+":J"+str(row_data_end)+")"),self.pd_cell_style_number_fill)
            ws_o.write(row_pos_o, 10, xlwt.Formula("SUM(K"+str(row_data_begin)+":K"+str(row_data_end)+")"),self.pd_cell_style_number_fill)
            ws_o.write(row_pos_o, 11, xlwt.Formula("SUM(L"+str(row_data_begin)+":L"+str(row_data_end)+")"),self.pd_cell_style_number_fill)
            ws_o.write(row_pos_o, 12, xlwt.Formula("SUM(M"+str(row_data_begin)+":M"+str(row_data_end)+")"),self.pd_cell_style_number_fill)
...
stock_report_sprepart_per_tgl_xls(
    'report.dym.stock.sparepart.pertgl.report.xls',
    'account.period',
    parser=dym_stock_report_sparepart_print_pertgl_xls)
