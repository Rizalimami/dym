...
class dym_sale_order(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context=None):
        super(dym_sale_order, self).__init__(cr, uid, name, context=context)
    ...
    def hari(self):
        data_1 = self.pool.get('sale.order').browse(self.cr, self.uid, self.ids).payment_term.id
        data_order = self.pool.get('sale.order').browse(self.cr, self.uid, self.ids).date_order
    ...
    def invoice(self):
        ...
        name_invoice=obj_sale_order.invoice_ids.number
...
report_sxw.report_sxw('report.rml.sale.order', 'sale.order', 'addons/dym_sale_order/report/dym_sale_order_report.rml', parser = dym_sale_order, header = False)