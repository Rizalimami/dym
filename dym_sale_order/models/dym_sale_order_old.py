...
class account_invoice_line_sale_order(osv.osv):
    ...
    def _get_amount_sale_line(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            sale_line_ids = self.pool.get('sale.order.line').search(cr, uid, [('invoice_lines','=',[line.id])])
...
class dym_sale_order(osv.osv):
    ...
    def onchange_partner_id_new(self, cr, uid, ids, part, division, branch_id, context=None):
        ...
        if not (part and division and branch_id):
            return {'value': {'pricelist_id':False,'partner_invoice_id': False, 'partner_shipping_id': False,  'payment_term': False, 'fiscal_position': False,'member':partner_obj.member}}
        if partner_obj:
            total_inv = self._invoice_total(cr,uid,ids,part,division)
            res['value'].update({'total_invoiced':total_inv,'total_invoiced_show':total_inv,'credit_limit_unit':partner_obj.credit_limit_unit,'credit_limit_sparepart':partner_obj.credit_limit_sparepart,'member':partner_obj.member})
...
#             current_pricelist = self.branch_id.pricelist_umum_purchase_id.id            
    ...
    def change_save_lost_order(self, cr, uid, ids, branch_dummy, product_id, lost_order_qty, qty_available, product_uom_qty, price_unit, context=None):
        ...
        if lost_order_qty > 0 and branch_dummy and product_id and lost_order_qty:
            ...
            self.pool.get('oned.lost.order').create(cr, uid, res)
            if qty_available > 0:
    ...
    def category_change(self, cr, uid, ids, categ_id, branch_id, division, pricelist_id, customer_id):
        ...
        return {'value':{'branch_dummy':branch_id,'division_dummy':division,'customer_dummy':customer_id},'domain':dom}
