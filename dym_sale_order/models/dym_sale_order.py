...
class dym_sale_order(osv.osv):
    ...
    def _get_default_branch(self,cr,uid,ids,context=None):
        ...
        res = False
        branch_id = user_obj.browse(cr,uid,uid).branch_id.id
        if branch_id:
            res = branch_id
        return res
    ...
    def _get_btn_delivery(self, cr, uid, ids, name, args, context=None):
        res = {}
        status = 'yes'
        so = self.browse(cr, uid, ids, context=context)
        invoice_ids = self.pool.get('account.invoice').search(cr, uid, [('origin','=',so.name)])
        invoice = self.pool.get('account.invoice').browse(cr, uid, invoice_ids)
        if (so.payment_term and so.payment_term.id != 1) or invoice.state == 'paid':
            status = 'yes'
        res[so.id] = status
        return res
    ...
    _columns = {
        ...
        'btn_delivery_show': fields.function(_get_btn_delivery, type='char', string='Button Delivery Show')
    }
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if 'partner_id' in vals:
            partner_id = obj_partner.browse(cr,uid,vals['partner_id'])
            if 'tipe_transaksi' in vals and vals['tipe_transaksi'] != 'pic':
                if partner_id.npwp and not partner_id.tipe_faktur_pajak :
                    raise osv.except_osv(('Perhatian !'), ("Tipe Faktur Pajak di master Customer belum dilengkapi"))
                elif not partner_id.npwp and partner_id.tipe_faktur_pajak != 'tanpa_fp':
                    raise osv.except_osv(('Perhatian !'), ("Nomor NPWP atau Tipe Faktur Pajak di master Customer belum dilengkapi"))
                # elif partner_id.npwp and partner_id.tipe_faktur_pajak == 'tanpa_fp':
                #     raise osv.except_osv(('Perhatian !'), ("Tipe Faktur Pajak tidak boleh diisi 'Tanpa Faktur Pajak' di master Customer jika ada NPWP")) 
                elif not partner_id.npwp and not partner_id.tipe_faktur_pajak:
                    raise osv.except_osv(('Perhatian !'), ("Tipe Faktur Pajak di master Customer belum dilengkapi")) 
        
                if partner_id.tipe_faktur_pajak == 'tanpa_fp':
                    vals['pajak_gunggung']=True
                    vals['pajak_generate']=False
                    vals['pajak_gabungan']=False
                elif partner_id.tipe_faktur_pajak == 'satuan':
                    vals['pajak_gunggung']=False
                    vals['pajak_generate']=True
                    vals['pajak_gabungan']=False
                elif partner_id.tipe_faktur_pajak == 'gabungan':
                    vals['pajak_gunggung']=False
                    vals['pajak_generate']=False
        ...
        for line in res.order_line:
            if res.tipe_transaksi == 'hotline' and line.qty_available > 0:
                raise osv.except_osv(('Perhatian !'), ("SO Hotline hanya untuk produk yang availablenya 0"))
            if (res.tipe_transaksi == 'reguler' or res.tipe_transaksi == 'pic') and line.qty_available <= 0:
                product_qty_0.append(line.product_id.name)
        
        if product_qty_0:
            raise osv.except_osv(('Perhatian !'), ("Product %s qty availablenya 0. Silahkan pilih product lain, jika tetap menghendaki product tersebut, lakukan SO Hotline." % ", ".join(product_qty_0)))   
        return sale_order
        ...
        for order in self.browse(cr,uid,ids):
            for line in order.order_line:
                if line.price_subtotal < 0:
                    raise osv.except_osv(('Perhatian !'), ("Sub Total Amount untuk baris %s tidak boleh minus! harap cek kembali data anda!" % line.name))
            if order.amount_total < 0:
                 raise osv.except_osv(('Perhatian !'), ("Total Amount tidak boleh minus! harap cek kembali data anda!"))
            product_qty_0 = []
            for line in order.order_line:
                if order.tipe_transaksi == 'hotline' and line.qty_available > 0:
                    raise osv.except_osv(('Perhatian !'), ("SO Hotline hanya untuk produk yang availablenya 0"))
                if (order.tipe_transaksi == 'reguler' or order.tipe_transaksi == 'pic') and line.qty_available <= 0:
                    product_qty_0.append(line.product_id.name)
            if product_qty_0:
                raise osv.except_osv(('Perhatian !'), ("Product %s qty availablenya 0. Silahkan pilih product lain, jika tetap menghendaki product tersebut, lakukan SO Hotline." % ", ".join(product_qty_0)))
        return res
    ...
    def _make_invoice(self, cr, uid, order, lines, context=None):
        ...
        for line in order.order_line:
            if line.categ_id.bisnis_unit:
                analytic_acc_1, analytic_acc_2, analytic_acc_3, analytic_acc_4 = self.pool.get('account.analytic.account').get_analytical(cr, uid, order.branch_id, 'Sparepart', line.categ_id, 4, 'Sparepart_Accesories')
            for disc in line.discount_line:
                if disc.tipe_diskon == 'percentage':
                    ...
                    if line.categ_id.bisnis_unit:
                        ...
                        per_potongan['discount_pelanggan_acc'] = per_potongan.get('discount_pelanggan_acc',0)+(total_diskon*line.product_uom_qty)
                else:
                    ...
                    if not line.categ_id.bisnis_unit:
                        ...
                        per_potongan['discount_external_acc'] = per_potongan.get('discount_external_acc',0)+(total_diskon_external*line.product_uom_qty)
                        per_potongan['discount_pelanggan_acc'] = per_potongan.get('discount_pelanggan_acc',0)+(total_diskon_pelanggan*line.product_uom_qty)
        ...
        per_potongan['discount'] = sum(line.discount_show for line in order.order_line if not line.categ_id.bisnis_unit) 
        per_potongan['discount_acc'] = sum(line.discount_show for line in order.order_line if line.categ_id.bisnis_unit) 
        per_potongan['discount_cash'] = sum(line.discount_cash for line in order.order_line if not line.categ_id.bisnis_unit)
        per_potongan['discount_lain'] = sum(line.discount_lain for line in order.order_line if not line.categ_id.bisnis_unit)
        per_potongan['discount_cash_acc'] = sum(line.discount_cash for line in order.order_line if line.categ_id.bisnis_unit)
        per_potongan['discount_lain_acc'] = sum(line.discount_lain for line in order.order_line if line.categ_id.bisnis_unit)
        ...
        if order.division=='Unit':
            ...
            if not order.order_line:
                raise osv.except_osv(
                            ...
                            _('Mohon isi detail penjualan.'))
            analytic_1_general, analytic_2_general, analytic_3_general, analytic_4_general = self.pool.get('account.analytic.account').get_analytical(cr, uid, order.branch_id, '', order.order_line[0].product_id.categ_id, 4, 'General')
            analytic_1, analytic_2, analytic_3, analytic_4 = self.pool.get('account.analytic.account').get_analytical(cr, uid, order.branch_id, '', line.categ_id, 4, 'Sparepart_Accesories')
            for line in order.order_line:
                if not line.categ_id.bisnis_unit:
                    analytic_1, analytic_2, analytic_3, analytic_4 = self.pool.get('account.analytic.account').get_analytical(cr, uid, order.branch_id, '', line.categ_id, 4, 'Sparepart_Accesories')
                    analytic_1_general, analytic_2_general, analytic_3_general, analytic_4_general = self.pool.get('account.analytic.account').get_analytical(cr, uid, order.branch_id, '', line.categ_id, 4, 'General')
        # if order.division=='Unit':
        #     analytic_1, analytic_2, analytic_3, analytic_4 = self.pool.get('account.analytic.account').get_analytical(cr, uid, order.branch_id, 'Unit', False, 4, 'Sales')
        # elif order.division=='Sparepart':
        ...
        for key, value in per_potongan.items():
            if value > 0:
                ...
                # Cek Account Potongan penjualan PIC
                if order.tipe_transaksi == 'pic':
                    if not obj_branch_config.dym_so_account_potongan_pic_id:
                        raise osv.except_osv(('Perhatian !'), ("Konfigurasi account potongan PIC belum lengkap!"))
        # invoice_line_discount = self.pool.get('account.invoice.line').search(cr,uid,[('name', 'like', 'Diskon')])
        # if not invoice_line_discount:
                if key=='discount':
                    invoice_lines.append({
                            ...
                            'account_id': obj_branch_config.so_account_potongan_langsung_id.id if order.tipe_transaksi!='pic' else obj_branch_config.dym_so_account_potongan_pic_id.id,
                            'invoice_id': inv_id,
                ...
                if key=='discount_pelanggan':
                    invoice_lines.append({
                            ...
                            'price_unit':price_unit,
                            'invoice_line_tax_id':tax,
                            'account_id': obj_branch_config.dym_so_account_discount_program_id.id if order.tipe_transaksi!='pic' else obj_branch_config.dym_so_account_potongan_pic_id.id,
                            'invoice_id': inv_id,
                            'analytic_1':analytic_1 or False,
                            'analytic_2':analytic_2 or False,
                            'analytic_3':analytic_3 or False,
                ...
                if key=='discount_external':
                    ...
                    invoice_pelunasan_line.append([0,False,{
                            ...
                            'price_unit':price_unit,
                            'invoice_line_tax_id':tax,
                            'account_id': obj_branch_config.dym_so_account_discount_program_external_id.id if order.tipe_transaksi!='pic' else obj_branch_config.dym_so_account_potongan_pic_id.id,
                            'invoice_id': inv_id,
                            'analytic_1': analytic_1 or False,
                            'analytic_2': analytic_2 or False,
                            'analytic_3': analytic_3 or False,
                ...
                if key=='discount_cash':
                    invoice_lines.append({
                            ...
                            'price_unit':price_unit,
                            'invoice_line_tax_id':tax,
                            'account_id': obj_branch_config.dym_so_account_discount_cash_id.id if order.tipe_transaksi!='pic' else obj_branch_config.dym_so_account_potongan_pic_id.id,
                            'invoice_id': inv_id,
                            'analytic_1':analytic_1 or False,
                            'analytic_2':analytic_2 or False,
                            'analytic_3':analytic_3 or False,
                ...
                if key=='discount_lain':
                    invoice_lines.append({
                            ...
                            'price_unit':price_unit,
                            'invoice_line_tax_id':tax,
                            'account_id': obj_branch_config.dym_so_account_discount_lainnya_id.id if order.tipe_transaksi!='pic' else obj_branch_config.dym_so_account_potongan_pic_id.id,
                            'invoice_id': inv_id,
                            'analytic_1':analytic_1 or False,
                            'analytic_2':analytic_2 or False,
                            'analytic_3':analytic_3 or False,
                ...
                if key=='discount_acc':
                    invoice_lines.append({
                            ...
                            'account_id': obj_branch_config.so_account_potongan_langsung_id.id if order.tipe_transaksi!='pic' else obj_branch_config.dym_so_account_potongan_pic_id.id,
                            'invoice_id': inv_id,
                            'analytic_1':analytic_acc_1 or False,
                            'analytic_2':analytic_acc_2 or False,
                            'analytic_3':analytic_acc_3 or False,
                            'account_analytic_id':analytic_acc_4 or False,
                        })
                ...
                if key=='discount_cash_acc':
                    invoice_lines.append({
                            'name': 'Diskon Cash Acc',
                            'quantity':1,
                            'origin':order.name,
                            'price_unit':price_unit,
                            'invoice_line_tax_id':tax,
                ...
                if key=='discount_lain_acc':
                    invoice_lines.append({
                            'name': 'Diskon Lainnya Acc',
                            'quantity':1,
                            'origin':order.name,
                            'price_unit':price_unit,
                            'invoice_line_tax_id':tax,
        ...
        for order in self.browse(cr,uid,ids):
            ...
            for lines in order.order_line:
                if lines.location_id:
                    move_ids = stock_move.search(cr, uid, [('origin','=',lines.order_id.name),('product_id','=',lines.product_id.id)])
                    move_browse = stock_move.browse(cr, uid, move_ids)
                    move_browse.write({'location_id': lines.location_id.id, 'sale_order_line_id': lines.id})
    ...
    def action_button_confirm(self, cr, uid, ids, context=None):
        ...
        if not(order.payment_term and len(order.payment_term.line_ids.ids) == 1 and order.payment_term.line_ids.days == 0 and order.payment_term.line_ids.days2 == 0 and order.payment_term.line_ids.value == 'balance'):
            if order.division == 'Unit':
                ...
                # else:
                #     sisa_credit = order.partner_id.credit_limit_unit - invoice_total
                #     order.partner_id.write({'credit_limit_unit':sisa_credit})
            elif order.division == 'Sparepart' :
                ...
                # else:
                #     sisa_credit = order.partner_id.credit_limit_sparepart - invoice_total
                #     order.partner_id.write({'credit_limit_sparepart':sisa_credit})
        ...
        # self.pool.get('stock.picking').do_transfer(cr, uid, [order.picking_ids.id], context)
...
#             current_pricelist = self.branch_id.pricelist_umum_purchase_id.id            
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        if product and 'division' in context and 'branch_id' in context:
            ...
            location_ids = product_id.get_location_ids()
            dom = res.get('domain', {})
            dom.update({
                        'location_id': [('id','in',location_ids),('branch_id','=',context['branch_id']),('usage','=','internal')]
                    })
            res["domain"] = dom
        else:
    ...
    _columns = {
        ...
        'tax_id_view': fields.many2many('account.tax', 'tax_view_account_tax_rel', 'tax_view_id', 'tax_id', type='many2many', string='Taxes'),
    ...
    def change_save_lost_order(self, cr, uid, ids, branch_dummy, product_id, lost_order_qty, qty_available, product_uom_qty, price_unit, customer_id, context=None):
        value = {}
        so = self.browse(cr, uid, ids, context=context)
        if lost_order_qty > 0 and branch_dummy and product_id and lost_order_qty:
            res = {'date':datetime.now().strftime('%Y-%m-%d'), 'branch_id': branch_dummy, 'product_id': product_id, 'lost_order_qty': lost_order_qty , 'tipe_dok': 'SO', 'no_dok': so.name, 'pemenuhan_qty': qty_available, 'het': price_unit, 'customer_id' : customer_id, 'rec_id': 0, 'rec_model':'sale.order.line'}
    ...
    def category_change(self, cr, uid, ids, categ_id, branch_id, division, pricelist_id, customer_id, tipe_transaksi):
