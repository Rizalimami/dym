...
from .dym_report_kartu_stock import dym_report_kartu_stock_print
from openerp.tools.translate import translate
import string

_ir_translation_name = 'report.kartu.stock'

class dym_report_kartu_stock_print_xls(dym_report_kartu_stock_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_kartu_stock_print_xls, self).__init__(cr, uid, name, context=context)
        move_obj = self.pool.get('stock.move')
        self.context = context
        wl_overview = move_obj._report_xls_kartu_stock_fields(cr, uid, context)
        #aris
        wl_overview_sparepart = move_obj._report_xls_kartu_stock_fields_sparepart(cr,uid, context)
...
class report_kartu_stock_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_kartu_stock_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_overview = {
            ...
            'chassis': {
                ...
                'totals': [1, 22, 'text', None]},
            'qty': {
                'header': [1, 22, 'text', _render("_('Qty in')")],
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            c_specs_o = [
                ('report_name', 1, 0, 'text', 'LAPORAN Kartu Stock ' + division),
            ]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(
                ws_o, row_pos_o, row_data, row_style=cell_style)
...
report_kartu_stock_xls(
    'report.Laporan Kartu Stock',
    'stock.move',
    parser = dym_report_kartu_stock_print_xls)
