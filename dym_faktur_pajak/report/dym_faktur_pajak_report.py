...
class dym_faktur_pajak_report(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context=None):
        super(dym_faktur_pajak_report, self).__init__(cr, uid, name, context=context)
    ...
    def faktur_name(self) :
        vals = self.pool.get('dym.faktur.pajak.out').browse(self.cr,self.uid,self.ids)
    ...
    def remark(self):
        ...
        model_id = self.pool.get('dym.remark').search(self.cr,self.uid,[
        ...
        model_brw = self.pool.get('dym.remark').browse(self.cr,self.uid,model_id)
        if vals.model_id.model == 'account.voucher' :
            remark = vals.keterangan
        else :
            remark = model_brw.remark
        if vals.model_id.model == 'dym.work.order' :
            ...
            if inv_ds :
                ...
                for x in invoice_data :
                    remark += ' ( '+str(x.number)+' )'
        elif vals.model_id.model == 'dym.asset.disposal' :
            ad_obj = self.pool.get('dym.asset.disposal')
            ad_search = ad_obj.browse(self.cr,self.uid,vals.transaction_id)
            inv_ad = invoice.search(self.cr,self.uid,[
                                                      ('origin','ilike',ad_search.name),
                                                      ('amount_tax','!=',False),
                                                      ('tipe','in',('customer','finco'))
                                                      ])
            if inv_ad :
                invoice_data = invoice.browse(self.cr,self.uid,inv_ad)
                for x in invoice_data :
                    remark += ' ( '+str(x.number)+' )'
        elif vals.model_id.model == 'sale.order' :
            so_obj = self.pool.get('sale.order')
            so_search = so_obj.browse(self.cr,self.uid,vals.transaction_id)
            inv_so = invoice.search(self.cr,self.uid,[
                                                      ('origin','ilike',so_search.name),
                                                      ('amount_tax','!=',False),
                                                      ])
            if inv_so :
                invoice_data = invoice.browse(self.cr,self.uid,inv_so)
                for x in invoice_data :
                    remark += ' ( '+str(x.number)+' )'
        return remark
    ...
    def faktur_pajak(self):
        ...
        obj_model_id = obj_model.search(self.cr, self.uid,[ ('model','=','dym.faktur.pajak') ])[0]
    ...
    def waktu_local(self):
        ...
        self.pool.get('dym.register.kwitansi.line').write(self.cr, self.uid,obj_voucher.kwitansi_id.id,{'state': 'printed','payment_id':obj_voucher.id,'amount':obj_voucher.amount})
...
report_sxw.report_sxw('report.rml.faktur_pajak', 'dym.faktur.pajak.out', 'addons/dym_faktur_pajak/report/dym_faktur_pajak_report.rml', parser = dym_faktur_pajak_report, header = False)