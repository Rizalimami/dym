...
class dym_report_piutang_wizard(orm.TransientModel):
    _name = 'dym.report.piutang.wizard'
    _description = 'Report Piutang Wizard'

    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if not context: context = {}
        res = super(dym_report_piutang_wizard, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = {
        ...
        'branch_ids': fields.many2many('dym.branch', 'dym_report_piutang_branch_rel', 'dym_report_piutang_wizard_id',
                                        'branch_id', 'Branch', copy=False),
        'partner_ids': fields.many2many('res.partner', 'dym_report_piutang_partner_rel', 'dym_report_piutang_wizard_id',
                                        'partner_id', 'Customer', copy=False, domain=[('customer','=',True)]),
        'account_ids': fields.many2many('account.account', 'dym_report_piutang_account_rel', 'dym_report_piutang_wizard_id',
                                        'account_id', 'Account', copy=False, domain=[('type','=','receivable')]),
        'journal_ids': fields.many2many('account.journal', 'dym_report_piutang_journal_rel', 'dym_report_piutang_wizard_id',
