...
from .dym_report_aging_hutang import dym_report_aging_hutang_print
from openerp.tools.translate import translate
import logging
_logger = logging.getLogger(__name__)
import string

_ir_translation_name = 'report.aging.hutang'

class dym_report_aging_hutang_print_xls(dym_report_aging_hutang_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_aging_hutang_print_xls, self).__init__(
            cr, uid, name, context=context)
        move_line_obj = self.pool.get('account.move.line')
        self.context = context
        wl_overview = move_line_obj._report_xls_aging_hutang_fields(
            cr, uid, context)
        tmpl_upd_overview = move_line_obj._report_xls_arap_overview_template(
            cr, uid, context)
        wl_overview_detail_pembayaran = move_line_obj._report_xls_aging_hutang_fields_detail_pembayaran(
            cr, uid, context)
...
class report_aging_hutang_xls(report_xls):

    def __init__(self, name, table, rml=False,
                 parser=False, header=True, store=False):
        super(report_aging_hutang_xls, self).__init__(
        ...
        self.col_specs_template_overview = {
            ...
            'partner_name': {
                ...
                'totals': [1, 22, 'text', None]},
            'payment_term': {
                'header': [1, 22, 'text', _render("_('Payment Term')")],
                'lines': [1, 0, 'text', _render("p['payment_term']")],
                'totals': [1, 22, 'text', None]},
            'account_code': {
            ...
            'analytic_4': {
                ...
                'totals': [1, 0, 'text', None]},
            'invoice_name': {
                'header': [1, 22, 'text', _render("_('No Register Invoice')")],
            ...
            'supplier_invoice_number': {
                ...
                'totals': [1, 22, 'text', None]},
            'date_aml': {
                'header': [1, 22, 'text', _render("_('Tgl Register Invoice')")],
            ...
            'bank': {
                ...
                'totals': [1, 22, 'text', None]},
            'an_rek': {
                'header': [1, 22, 'text', _render("_('an supplier direkening')")],
            ...
            'tgl_retur': {
                ...
                'totals': [1, 22, 'text', None]},
            'no_retur': {
                'header': [1, 22, 'text', _render("_('No Bukti Retur')")],
                'lines': [1, 0, 'text', _render("p['no_retur']")],
                'totals': [1, 22, 'text', None]},
            'total_retur': {
                'header': [1, 22, 'text', _render("_('Total Retur')")],
            ...
            'pay_no': {
                'header': [1, 22, 'text', _render("_('Nomor Dokumen')")],
                'lines': [1, 0, 'text', _render("p['pay_no']")],
                'totals': [1, 22, 'text', None]},
            'pay_date': {
                'header': [1, 22, 'text', _render("_('Tanggal Dokumen')")],
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            report_name = ' '.join(
                [_('LAPORAN Aging Hutang Per Tanggal'), _(str(datetime.today().date())),
                 _p.report_info])
            c_specs_o = [
                ('report_name', 1, 0, 'text', report_name),
            ]
...
report_aging_hutang_xls('report.Laporan Aging Hutang', 'account.move.line', parser = dym_report_aging_hutang_print_xls)
