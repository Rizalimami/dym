...
class dym_subsidi_barang(osv.osv):
    _name = 'dym.subsidi.barang'
    _description = "Subsidi Barang"
    ...
    def _get_order(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool.get('dym.subsidi.barang.line').browse(cr, uid, ids, context=context):
    ...
    _columns = { 
                ...
                'product_template_id':fields.related("product_template", string="Direct Gift", relation='product.template',type='many2one',store=False),

                'product_template':fields.many2one('product.template',"Direct Gift",required=True),
                'nilai_promo':fields.function(_get_max, string="Nilai Promo",
                store={
                        'dym.subsidi.barang': (lambda self, cr, uid, ids, c={}: ids, ['subsidi_barang_line'], 10),
                        'dym.subsidi.barang.line': (_get_order, ['diskon_ahm', 'diskon_md', 'diskon_dealer', 'diskon_finco', 'diskon_others'], 10),
                    },
                    multi='sums',help="Subtotal Diskon."),
                'subsidi_barang_line':fields.one2many('dym.subsidi.barang.line','subsidi_barang_id',"Subsidi Barang",required=True),       
                ...
                'area_id':fields.many2one('dym.area','Area',required=True),
    ...
    def create(self,cr,uid,vals,context=None):
        ...
        return super(dym_subsidi_barang, self).create(cr, uid, vals, context=context)
    
    def unlink(self, cr, uid, ids, context=None):
         for item in self.browse(cr, uid, ids, context=context):
             if item.state != 'draft':
                 raise osv.except_osv(('Perhatian !'), ("Subsidi Barang tidak bisa didelete !"))
         return super(dym_subsidi_barang, self).unlink(cr, uid, ids, context=context)
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(dym_subsidi_barang, self).copy(cr, uid, id, default=default, context=context)
     
class dym_subsidi_barang_line(osv.osv):
    _name = 'dym.subsidi.barang.line'
    ...
    _columns = { 
                'subsidi_barang_id':fields.many2one('dym.subsidi.barang',"Subsidi Barang",required=True),
    ...
    def _get_domain_product(self,cr,uid,ids,division,context=None):
        domain = {} 
        if division:
