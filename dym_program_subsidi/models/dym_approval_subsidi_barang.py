...
class dym_subsidi_barang(osv.osv):
    ...
    def dym_request_approval(self, cr, uid, ids, context=None):
    ...
    def dym_approval(self, cr, uid, ids, context=None):
        obj_subsidi_barang = self.browse(cr, uid, ids, context=context)
        if not obj_subsidi_barang.subsidi_barang_line:
            raise osv.except_osv(('Perhatian !'), ("Line belum diisi"))
        approval_sts = self.pool.get("dym.approval.matrixbiaya").approve(cr, uid, ids, obj_subsidi_barang)
        if approval_sts == 1:
    ...
    def dym_perpanjang_periode(self, cr, uid, ids, context=None):
    ...
    def dym_revise(self, cr, uid, ids, context=None):    
             val = self.browse(cr, uid, ids)      
             so = self.pool.get('dealer.sale.order.line.brgbonus.line')
             so_search= so.search(cr,uid,[
                                          ('barang_subsidi_id','=',val.name)
