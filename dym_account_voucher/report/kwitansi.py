...
class dym_k_account_voucher(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context=None):
        super(dym_k_account_voucher, self).__init__(cr, uid, name, context=context)
    ...
    def kwitansi(self):
        ...
        kwitansi = self.pool.get('dym.register.kwitansi.line').search(self.cr,self.uid,[
                                                                                        ('payment_id','=',vals.id),
                                                                                        ('state','=','printed'),
                                                                                        ('reason','=',False)
                                                                                        ])
        if kwitansi :
            prev_kwitansi = self.pool.get('dym.register.kwitansi.line').browse(self.cr,self.uid,kwitansi)
            for x in prev_kwitansi :        
                self.pool.get('dym.register.kwitansi.line').write(self.cr,self.uid,x.id,{'state':'cancel','reason':str(vals.reason_cancel_kwitansi)})
        self.pool.get('dym.register.kwitansi.line').write(self.cr, self.uid,vals.kwitansi_id.id, {
                                                                                                  'payment_id':vals.id,                                                                                                 
                                                                                                  'state':'printed'
                                                                                                  })
...
report_sxw.report_sxw('report.rml.kwitansi', 'account.voucher', 'addons/account_voucher/report/kwitansi.rml', parser = dym_k_account_voucher, header = False)