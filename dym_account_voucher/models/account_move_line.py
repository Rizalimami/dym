...
from openerp import models, fields, api, SUPERUSER_ID, workflow
...
class dym_account_move_line(models.Model):
    ...
    def reconcile(self, cr, uid, ids, type='auto', writeoff_acc_id=False, writeoff_period_id=False, writeoff_journal_id=False, context=None):
        ...
        partner_ids = list(set([x.partner_id.id for x in lines]))

        if len(partner_ids) > 1:
            # Collect all tax account_id
            wh_tax_ids = self.pool.get('account.tax.withholding').search(cr, uid, [('company_id','=',lines[0].company_id.id)])
            wh_taxes = self.pool.get('account.tax.withholding').read(cr, uid, wh_tax_ids, ['account_id'], context=context)
            wh_tax_ids = [t['account_id'][0] for t in wh_taxes]

            sql = ('SELECT account_id, reconcile_id '\
                       'FROM account_move_line '\
                       'WHERE id IN %s AND account_id NOT IN %s'\
                       'GROUP BY account_id,reconcile_id' % (tuple(ids), tuple(wh_tax_ids),))
        else:
            sql = ('SELECT account_id, reconcile_id '\
                       'FROM account_move_line '\
                       'WHERE id IN %s'\
                       'GROUP BY account_id,reconcile_id' % (tuple(ids),))
        ...
        for rec in self.browse(cr, uid, ids, context=context):
            self.write(cr, uid, [rec.id], {'reconcile_id': r_id, 'reconcile_partial_id': False}, context=reconcile_context)
