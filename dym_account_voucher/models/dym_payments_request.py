...
class account_voucher_pr(models.Model):
    ...
    def branch_onchange(self):
        ...
        if self.branch_id :
            ...
            journal_id=branch_config.dym_payment_request_account_id.id
            account_id=branch_config.dym_payment_request_account_id.default_credit_account_id.id
            if not journal_id :
    ...
    payments_request_ids = fields.One2many('account.voucher.line', 'voucher_id', readonly=True, copy=True)
...
class account_voucher_line(models.Model):
    ...
    def name_onchange(self,name,branch_id,division):
        ...
        dict=self.env['dym.account.filter'].get_domain_account('payments_request')
        edi_doc_list.extend(dict)      
        dom['account_id'] = edi_doc_list
        return {'domain':dom}     
    
class dym_payments_request_type(models.Model):
