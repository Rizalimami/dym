...
class dym_penyerahan_bpkb(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context=None):
        super(dym_penyerahan_bpkb, self).__init__(cr, uid, name, context=context)
    ...
    def kwitansi(self):
        vals = self.pool.get('dym.penyerahan.bpkb').browse(self.cr,self.uid,self.ids)
        obj_model = self.pool.get('ir.model')
        obj_model_id = obj_model.search(self.cr, self.uid,[ ('model','=','dym.penyerahan.bpkb') ])[0]
        obj_ir = self.pool.get('ir.actions.report.xml').search(self.cr, self.uid,[('report_name','=','rml.dym.penyerahan.bpkb')])
...
report_sxw.report_sxw('report.rml.dym.penyerahan.bpkb', 'dym.penyerahan.bpkb', 'addons/dym_penyerahan_stnk/report/dym_penyerahan_bpkb_report.rml', parser = dym_penyerahan_bpkb, header = False)