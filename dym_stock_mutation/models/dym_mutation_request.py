...
class dym_mutation_request(models.Model):
    _name = "dym.mutation.request"
    _description = "Mutation Request"
    

    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if not context: context = {}
        res = super(dym_mutation_request, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    def _get_picking_ids(self):
        ...
        for sd in self.env['dym.stock.distribution'].sudo().search([('request_id','=',self.id)]):
            ids_sd.append(sd.id)
        for mo in self.env['dym.mutation.order'].sudo().search([('distribution_id','in',ids_sd)]):
    ...
    branch_id = fields.Many2one('dym.branch', 'Branch Requester')
    ...
    type_id = fields.Many2one('dym.purchase.order.type', 'Type Request', required=True)
    user_id = fields.Many2one('res.users', 'Responsible')
    description = fields.Char('Description')
    request_line = fields.One2many('dym.mutation.request.line', 'request_id', 'Mutation Line')
    amount_total = fields.Float(string='Total', digits=dp.get_precision('Account'),
                                store=True, compute='_compute_amount')
    start_date = fields.Date('Start Date')
    end_date = fields.Date('Scheduled Date')
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        vals['name'] = self.pool.get('ir.sequence').get_per_branch(cr, uid, vals['branch_id'], 'MRE', division=vals['division'])
        vals['rel_date'] = self._get_default_date(cr,uid)
        return super(dym_mutation_request, self).create(cr, uid, vals, context=context)
    ...
    def wkf_action_cancel(self):
        ...
        distribution_id = self.env['dym.stock.distribution'].sudo().search([('request_id','=',self.id)])
        distribution_id.write({'state': 'cancel'})
    
    def action_distribution_create(self):
        obj_branch = self.env['dym.branch'].sudo().search([('id','=',int(self.branch_sender_id))])
        distribution_vals = {
        ...
        distribution_id = self.env['dym.stock.distribution'].sudo().create(distribution_vals)
        for line in self.request_line :
            ...
            self.env['dym.stock.distribution.line'].sudo().create(distribution_line_vals)
    ...
    @api.model
    def is_done(self):
        obj_sd = self.env['dym.stock.distribution'].search([('request_id','=',self.id)])
    ...
    def type_id_change(self):
        type = self.env['dym.purchase.order.type'].browse(self.type_id.id)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(dym_mutation_request, self).unlink(cr, uid, ids, context=context)
    ...
    def view_picking(self, cr, uid, ids, context=None):
        ...
        for sd in self.pool.get('dym.stock.distribution').search(cr, SUPERUSER_ID, [('request_id','=',obj_me.id)]):
            ids_sd.append(sd)
        for mo in self.pool.get('dym.mutation.order').search(cr, SUPERUSER_ID, [('distribution_id','in',ids_sd)]):
            mo_id = self.pool.get('dym.mutation.order').browse(cr, SUPERUSER_ID, mo)
    ...
    """
    update dym_mutation_request
    set branch_sender_id = (select id from dym_branch where dym_branch.code = dym_mutation_request.branch_sender_id)
    """
    
class dym_mutation_request_line(models.Model):
    _name = "dym.mutation.request.line"
    ...
    request_id = fields.Many2one('dym.mutation.request', 'Request')
    ...
    def product_id_change(self, cr, uid, ids, product_id, branch_id, branch_sender_id, division, template_id=False, context=None):
        ...
        if template_id:
            ...
            if obj_product and template_id != obj_product.product_tmpl_id.id:
        ...
        if product_id :
            branch = self.pool.get('dym.branch').browse(cr, SUPERUSER_ID, int(branch_sender_id))
            obj_product = self.pool.get('product.product').browse(cr, uid, product_id)
            ...
            cost_quant = 0.0
            price_id = self.pool.get('product.price.branch').search(cr, uid, [('warehouse_id','=', branch.warehouse_id.id), ('product_id','=', product_id)])
            if price_id:
                cost_quant = self.pool.get('product.price.branch').browse(cr, uid, price_id[0]).cost
            value['unit_price'] = cost_quant
