...
class dym_reject_approval_stock_mutation(models.TransientModel):
    _name = "dym.reject.approval.stock.mutation"
    ...
    def dym_reject_approval(self, context=None):
        ...
        line = self.env['dym.mutation.request'].browse(po_id,context=context)
        objek = False
                    ...
                    if y == x.group_id :
                        ...
                        for z in line.approval_ids :
                            if z.reason == False :
                                ...
                                self.env['dym.mutation.request'].write( {'state':'draft','approval_state':'r'})
        if objek == False :
            raise exceptions.ValidationError("User tidak termasuk group approval")
