...
class dym_mutation_order(osv.osv):
    _name = "dym.mutation.order"
    _description = "Mutation Order"
    
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if not context: context = {}
        res = super(dym_mutation_order, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    def _get_order(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool.get('dym.mutation.order.line').browse(cr, uid, ids, context=context):
    ...
    def _get_picking_ids(self, cr, uid, ids, field_names, args, context=None):
        ...
        for po_id in ids:
            res[po_id] = []
        query = """
        SELECT p.id, po.id FROM stock_picking p, dym_mutation_order po
    ...
    def _get_default_date(self,cr,uid,context=None):
        return self.pool.get('dym.branch').get_default_date_model(cr,uid,context=context)
    ...
    _columns = {
                ...
                'order_line' : fields.one2many('dym.mutation.order.line', 'order_id', 'Mutation Line'),
                'amount_total' : fields.function(_compute_amount, string='Total', digits_compute=dp.get_precision('Account'),
                                                 store={
                                                        'dym.mutation.order': (lambda self, cr, uid, ids, c={}: ids, ['order_line'], 10),
                                                        'dym.mutation.order.line': (_get_order, ['unit_price', 'qty', 'subtotal'], 10),
                                                        },
                                                 multi='sums', help="The total amount."),
                'start_date' : fields.date('Start Date'),
                'end_date' : fields.date('End Date'),
                'distribution_id' : fields.many2one('dym.stock.distribution', 'Stock Distribution', required=True, ondelete='restrict'),
    ...
    def create(self, cr, uid, vals, context=None):
        vals['name'] = self.pool.get('ir.sequence').get_per_branch(cr, uid, vals['branch_id'], 'MOR', division=vals['division'])
        vals['date'] = self._get_default_date(cr, uid, context)
        return super(dym_mutation_order, self).create(cr, uid, vals, context=context)
    
    def _prepare_order_line_move(self, cr, uid, order, order_line, picking_id, context=None):
        res = []
        undelivered_value = 0
        warehouse = self.pool.get('dym.branch').browse(cr, uid, order.branch_id.id).warehouse_id
        if warehouse :
            ...
            if warehouse.interbranch_out_type_id.branch_id == order.branch_id :
                if warehouse.interbranch_out_type_id.default_location_dest_id.usage == 'transit' :
                    ...
                    if default_location_search:
                        ...
                        location_id = default_location_brw.location_id.id
            ...
            if order_line.product_id.type in ('product', 'consu'):
                for vals in self._prepare_order_line_move(cr, uid, order, order_line, picking_id, context=context):
                    if vals['product_uom_qty'] > 0 :
                        ...
                        stock_move.write(cr, uid, move, {'price_unit': order_line.unit_price})
    ...
    def action_picking_create(self, cr, uid, ids, context=None):
        ...
        warehouse = self.pool.get('dym.branch').browse(cr, uid, obj_me.branch_id.id).warehouse_id
        if warehouse :
            picking_type_id = warehouse.interbranch_out_type_id.id
        else :
    ...
    def get_stock_available(self, cr, uid, ids, id_product, id_branch, context=None):
        ...
        if not ids_location:
            ...
            raise osv.except_osv(('Perhatian !'), ("tidak ditemukan lokasi internal di branch %s!" %(branch.name)))
        cr.execute("""
        SELECT
    ...
    def view_picking(self, cr, uid, ids, context=None):
        ...
        for picking in self.pool.get('stock.picking').search(cr, SUPERUSER_ID, [
                                                                                ('origin','ilike',str(mutation_order_id.name)),
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(dym_mutation_order, self).unlink(cr, uid, ids, context=context)
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        for sd_line in mo_id.distribution_id.distribution_line :
            sd_line.write({'qty':sd_line.qty - product_qty[sd_line.product_id]})
    
class dym_mutation_order_line(osv.osv):
    _name = "dym.mutation.order.line"
    ...
    _columns = {
                'order_id' : fields.many2one('dym.mutation.order', 'Mutation'),
                'product_id' : fields.many2one('product.product', 'Variant'),
                'description' : fields.text('Description'),
                'qty' : fields.float('Qty MO', digits=(10,0)),
                ...
                'unit_price_show' : fields.function(_get_price, string='Unit Cost'),
                ...
                'rel_distribution_id' : fields.related('order_id', 'distribution_id', string='Stock Distribution', type='many2one', relation='dym.stock.distribution'),
                'performance_hpp': fields.float('Performance HPP', digits=(10,0)),
                'template_id': fields.related('product_id', 'product_tmpl_id', relation='product.template', type='many2one', string='Tipe'),
                }
    ...
    def qty_change(self, cr, uid, ids, qty, product_id, distribution, context=None):
        ...
        value = {}
        quantity = {}
        distribution_id = self.pool.get('dym.stock.distribution').browse(cr, uid, distribution)
        obj_me = self.browse(cr, uid, ids, context=context)
