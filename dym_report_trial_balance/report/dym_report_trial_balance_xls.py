...
from .dym_report_trial_balance import dym_trial_balance_report_print
from openerp.tools.translate import translate
import logging
_logger = logging.getLogger(__name__)
import time

_ir_translation_name = 'dym.report.trial.balance'

class dym_trial_balance_print_xls(dym_trial_balance_report_print):

    def __init__(self, cr, uid, name, context):
        super(dym_trial_balance_print_xls, self).__init__(
            cr, uid, name, context=context)
        moveline_obj = self.pool.get('account.move.line')
        self.context = context
        wl_overview = moveline_obj._report_xls_trial_balance_fields(
            cr, uid, context)
        wl_overview_consol = moveline_obj._report_xls_trial_balance_fields_consol(
            cr, uid, context)
        ...
        self.localcontext.update({
            ...
            'wanted_list_overview_consol': wl_overview_consol,
...
class trial_balance_report_xls(report_xls):
    ...
    def __init__(self, name, table, rml=False,
        ...
        self.col_specs_template_overview = {
            ...
            'account_name': {
                ...
                'totals': [1, 0, 'text', None]},
            'account_type': {
                'header': [1, 20, 'text', _render("_('Tipe Account')"),None,self.rh_cell_style_center],
                'second_header' : [1, 20, 'text', None],
                'lines': [1, 0, 'text', _render("p['account_type'] or 'n/a'")],
                'totals': [1, 0, 'text', None]},
            'analytic_combination': {
                'header': [1, 0, 'text', _render("_('Analytic Combination')"),None,self.rh_cell_style_center],
                'second_header' : [1, 20, 'text', None],
                'lines': [1, 0, 'text', _render("p['analytic_combination'] or ''")],
                'totals': [1, 0, 'text', None]},
            'analytic_1': {
                'header': [1, 0, 'text', _render("_('Analytic Company')"),None,self.rh_cell_style_center],
                'second_header' : [1, 20, 'text', None],
                'lines': [1, 0, 'text', _render("p['analytic_1'] or ''")],
                'totals': [1, 0, 'text', None]},
            'analytic_2': {
                'header': [1, 0, 'text', _render("_('Analytic Bisnis Unit')"),None,self.rh_cell_style_center],
                'second_header' : [1, 20, 'text', None],
                'lines': [1, 0, 'text', _render("p['analytic_2'] or ''")],
                'totals': [1, 0, 'text', None]},
            'analytic_3': {
                'header': [1, 0, 'text', _render("_('Analytic Branch')"),None,self.rh_cell_style_center],
                'second_header' : [1, 20, 'text', None],
                'lines': [1, 0, 'text', _render("p['analytic_3'] or ''")],
                'totals': [1, 0, 'text', None]},
            'analytic_4': {
                'header': [1, 0, 'text', _render("_('Analytic Cost Center')"),None,self.rh_cell_style_center],
                'second_header' : [1, 20, 'text', None],
            ...
            'mutasi_debit': {
                ...
                'totals': [1, 0, 'text', None]},  
            'mutasi_credit': {
                'header': [1, 20, 'text', _render("_('NET')"),None,self.rh_cell_style_center],
                'second_header': [1, 20, 'text', _render("_('Credit')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'number', _render("p['mutasi_credit'] or ''"),None,self.pd_cell_style_decimal],
                'totals': [1, 0, 'text', None]},  
            'debit_neraca': {
                'header': [2, 40, 'text', _render("_('Eliminasi')"),None,self.rh_cell_style_center],
                'second_header': [1, 20, 'text', _render("_('Debit')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'number', _render("p['debit_neraca'] or ''"),None,self.pd_cell_style_decimal],
                'totals': [1, 0, 'text', None]}, 
            'credit_neraca': {
                'header': [2, 40, 'text', _render("_('Konsolidasi')"),None,self.rh_cell_style_center],
                'second_header': [1, 20, 'text', _render("_('Credit')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'number', _render("p['credit_neraca'] or ''"),None,self.pd_cell_style_decimal],
                'totals': [1, 0, 'text', None]},         
            'net': {
                'header': [1, 20, 'text', _render("_('Konsolidasi NET')"),None,self.rh_cell_style_center],
                'second_header' : [1, 20, 'text', None],
                'lines': [1, 0, 'number', _render("p['net'] or ''"),None,self.pd_cell_style_decimal],
                'totals': [1, 0, 'text', None]},  
            'elim_debit': {
                'header': [1, 12, 'text', None, None, self.pd_cell_style],
                'second_header': [1, 20, 'text', _render("_('Debit')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'number', _render("p['elim_debit'] or ''"),None,self.pd_cell_style_decimal],
                'totals': [1, 0, 'text', None]},  
            'elim_credit': {
                'header': [1, 12, 'text', None, None, self.pd_cell_style],
                'second_header': [1, 20, 'text', _render("_('Credit')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'number', _render("p['elim_credit'] or ''"),None,self.pd_cell_style_decimal],
                'totals': [1, 0, 'text', None]},  
            'consol_debit': {
                'header': [1, 12, 'text', None, None, self.pd_cell_style],
                'second_header': [1, 20, 'text', _render("_('Debit')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'number', _render("p['consol_debit'] or ''"),None,self.pd_cell_style_decimal],
                'totals': [1, 0, 'text', None]},  
            'consol_credit': {
                'header': [1, 12, 'text', None, None, self.pd_cell_style],
                'second_header': [1, 20, 'text', _render("_('Credit')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'number', _render("p['consol_credit'] or ''"),None,self.pd_cell_style_decimal],
                'totals': [1, 0, 'text', None]},    
            'consol_net': {
                'header': [1, 12, 'text', None, None, self.pd_cell_style],
                'second_header' : [1, 20, 'text', None],
                'lines': [1, 0, 'number', _render("p['consol_net'] or ''"),None,self.pd_cell_style_decimal],
                'totals': [1, 0, 'text', None]},  
        }                                   
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            if r['konsolidasi'] == True:
                wanted_list_overview = _p.wanted_list_overview_consol
            ...
            c_specs_o = [
                ('title', 1, 20, 'text', r['jenis_laporan']),
            ]
            ...
            if r['konsolidasi'] == False:
                for col in c_specs_o:
                    if col[4] in ('Eliminasi','Konsolidasi','Konsolidasi NET'):
                        col[1] = 1
                        col[2] = 12
                        col[4] = ''
                        col[6] = self.pd_cell_style
            ...
            c_specs_o = map(lambda x: self.render(x, self.col_specs_template_overview, 'second_header',render_space={'_': _p._}),wanted_list_overview)
            ...
            ws_o.write(row_pos_o, 10, "Total",self.ph_cell_style)   
            ws_o.write(row_pos_o, 11, xlwt.Formula("SUM(L"+str(row_data_begin)+":L"+str(row_data_end)+")"),self.pd_cell_style_decimal_fill)
            ws_o.write(row_pos_o, 12, xlwt.Formula("SUM(M"+str(row_data_begin)+":M"+str(row_data_end)+")"),self.pd_cell_style_decimal_fill)
            ws_o.write(row_pos_o, 13, xlwt.Formula("SUM(N"+str(row_data_begin)+":N"+str(row_data_end)+")"),self.pd_cell_style_decimal_fill)    
            ...
            if r['konsolidasi'] == True:
                ...
                ws_o.write(row_pos_o, 19, xlwt.Formula("SUM(T"+str(row_data_begin)+":T"+str(row_data_end)+")"),self.pd_cell_style_decimal_fill)
                ws_o.write(row_pos_o, 20, xlwt.Formula("SUM(U"+str(row_data_begin)+":U"+str(row_data_end)+")"),self.pd_cell_style_decimal_fill)  
                ws_o.write(row_pos_o, 21, xlwt.Formula("SUM(V"+str(row_data_begin)+":V"+str(row_data_end)+")"),self.pd_cell_style_decimal_fill)  
                ws_o.write(row_pos_o, 22, xlwt.Formula("SUM(W"+str(row_data_begin)+":W"+str(row_data_end)+")"),self.pd_cell_style_decimal_fill)  
            ws_o.write(row_pos_o+1, 0, _p.report_date+" "+username)



trial_balance_report_xls(
    'report.dym_report_trial_balance_xls',
    'account.move.line',
    parser=dym_trial_balance_print_xls)
