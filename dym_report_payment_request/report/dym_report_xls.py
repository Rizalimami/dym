...
from .dym_report import dym_report_payment_request_print
from openerp.tools.translate import translate
import logging
_logger = logging.getLogger(__name__)
import string

_ir_translation_name = 'report.payment.request'

class dym_report_payment_request_print_xls(dym_report_payment_request_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_payment_request_print_xls, self).__init__(
            cr, uid, name, context=context)
        move_line_obj = self.pool.get('account.voucher')
        self.context = context
        wl_overview = move_line_obj._report_xls_payment_request_fields(
            cr, uid, context)
...
class report_payment_request_xls(report_xls):

    def __init__(self, name, table, rml=False,
                 parser=False, header=True, store=False):
        super(report_payment_request_xls, self).__init__(
        ...
        self.col_specs_template_overview = {
           ...
           'payment_request_type': {
               ...
               'lines': [1, 0, 'text', _render("p['payment_request_type']")],
               'totals': [1, 22, 'text', None]},
           'no_document': {
               'header': [1, 22, 'text', _render("_('No Document')")],
               'lines': [1, 0, 'text', _render("p['no_document']")],
               'totals': [1, 22, 'text', None]},
           'tgl_document': {
               'header': [1, 22, 'text', _render("_('Tgl Document')")],
               'lines': [1, 0, 'text', _render("p['tgl_document']")],
               'totals': [1, 22, 'text', None]},
            'total': {
                'header': [1, 22, 'text', _render("_('Total Payment Request')")],
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            report_name = ' '.join(
                [_('LAPORAN Payment Request Per Tanggal'), _(str(datetime.today().date())),
                 _p.report_info])
            c_specs_o = [
                ('report_name', 1, 0, 'text', report_name),
            ]
...
report_payment_request_xls('report.Laporan Payment Request', 'account.voucher', parser = dym_report_payment_request_print_xls)
