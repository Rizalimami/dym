...
class dym_report_payment_request_print(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        if context is None:
            context = {}
        super(dym_report_payment_request_print, self).__init__(
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        report_payment_request = {
            'type': 'payable',
            'title': '',
            'title_short': title_short_prefix + ', ' + _('Laporan Payment Request')}
        ...
        query_start = "SELECT av.id as id_ai, " \
            ...
            "av.payment_request_type as payment_request_type, " \
            "av.transaksi as transaksi, " \
            "av.no_document as no_document, " \
            "av.tgl_document as tgl_document, " \
            ...
            "where 1=1 and av.state = 'posted' and j.type in ('purchase','purchase_refund') and av.type = 'purchase' "
        ...
        reports = [report_payment_request]
            ...
            if all_lines:
                ...
                p_map = map(
                    lambda x: {
                        ...
                        'total': x['total'],
                        'transaksi': str(x['transaksi']) if x['transaksi'] != None else '',
                        'payment_request_type': str(x['payment_request_type']) if x['payment_request_type'] != None else '',
                        'no_document': str(x['no_document']) if x['no_document'] != None else '',
                        'tgl_document': str(x['tgl_document']) if x['tgl_document'] != None else '',
                        'state': x['state'],},
                ...
                for p in p_map:
                   if p['id_ai'] not in map(
                       ...
                       if analytic:
                           ...
                           move_line = ai.move_ids.filtered(lambda r: r.account_id.id == ai.account_id.id and r.credit > 0)
                           residual = 0
                           if move_line:
...
#                              'b': 3})
        ...
        if not reports :
            reports = [{'title_short': 'Laporan Payment Request', 'type': ['out_invoice','in_invoice','in_refund','out_refund'], 'id_ai':
                            [{'total': 0,
                              'date': 'NO DATA FOUND',
                              ...
                              'payment_request_type': 'NO DATA FOUND',
                              'no_document': 'NO DATA FOUND',
                              'tgl_document': 'NO DATA FOUND',
                              'no': 0,
                              'residual':0,
        ...
        super(dym_report_payment_request_print, self).set_context(
...
class wrapped_vat_declaration_print(orm.AbstractModel):
    _name = 'report.dym_report_payment_request.report_payment_request'
    _inherit = 'report.abstract_report'
    _template = 'dym_report_payment_request.report_payment_request'
    _wrapped_report_class = dym_report_payment_request_print
