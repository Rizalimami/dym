...
from .dym_report_rworsorcpa import dym_report_rworsorcpa_print
from openerp.tools.translate import translate
import string

class dym_report_rworsorcpa_print_xls(dym_report_rworsorcpa_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_rworsorcpa_print_xls, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'datetime': datetime,
            'wanted_list_overview': self.pool.get('dealer.spk')._report_xls_rworsorcpa_fields(cr, uid, context),
            '_': self._,
        })

    def _(self, src):
        lang = self.context.get('lang', 'en_US') 
        return translate(self.cr, 'report.rworsorcpa', 'report', lang, src) or src

class report_rworsorcpa_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_rworsorcpa_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_overview = {
            ...
            'branch': {
                ...
                'lines': [1, 0, 'text', _render("p['branch'] or 'n/a'")],
            ...
            'tipe_konsumen': {
                'header': [1, 22, 'text', _render("_('Tipe Konsumen')")],
            ...
            'o_date': {
                ...
                'lines': [1, 0, 'text', _render("p['o_date']")],
                'totals': [1, 22, 'number', None]},
            'total': {
            ...
            'cpa': {
                'header': [1, 22, 'text', _render("_('CPA')")],
                'lines': [1, 0, 'text', _render("p['cpa']")],
                'totals': [1, 22, 'number', None]},
            'cpa_date': {
                'header': [1, 22, 'text', _render("_('CPA Date')")],
                'lines': [1, 0, 'text', _render("p['cpa_date']")],
                'totals': [1, 22, 'number', None]},
            'cpa_total': {
                'header': [1, 22, 'text', _render("_('CPA Total')")],
                'lines': [1, 0, 'number', _render("p['cpa_total']"), None, self.pd_cell_style_decimal],
                'totals': [1, 22, 'number', _render("p['cpa_total']"), None, self.rt_cell_style_decimal]},
        }
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ws_o = wb.add_sheet('Laporan WOR SOR - CPA')
            ...
            c_specs_o = [('report_name', 1, 0, 'text', 'Laporan WOR/SOR - CPA')]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(ws_o, row_pos_o, row_data, row_style=cell_style)
...
report_rworsorcpa_xls('report.Laporan WOR/SOR - CPA', 'dealer.spk', parser = dym_report_rworsorcpa_print_xls)
