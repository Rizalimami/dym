...
class dym_report_request_trackingpco_wizard(orm.TransientModel):
    _name = 'dym.report.request.trackingpco.wizard'
    _description = 'Report User Request Tracking PCO Wizard'

    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if not context: context = {}
        res = super(dym_report_request_trackingpco_wizard, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = {
        ...
        'branch_ids': fields.many2many('dym.branch', 'dym_report_request_trackingpco_branch_rel', 'dym_report_request_trackingpco_wizard_id',
                                        'branch_id', 'Branch', copy=False),
        'partner_ids': fields.many2many('res.partner', 'dym_report_request_trackingpco_partner_rel', 'dym_report_request_trackingpco_wizard_id',
                                        'partner_id', 'Customer', copy=False, domain=[('customer','=',True)]),
        'account_ids': fields.many2many('account.account', 'dym_report_request_trackingpco_account_rel', 'dym_report_request_trackingpco_wizard_id',
                                        'account_id', 'Account', copy=False, domain=[('type','=','receivable')]),
        'journal_ids': fields.many2many('account.journal', 'dym_report_request_trackingpco_journal_rel', 'dym_report_request_trackingpco_wizard_id',
...
#             journal_ids = self.pool.get('account.journal').search(cr, uid, [('type','in',['purchase','sale_refund'])])
        ...
        if context.get('xls_export'):
            return {'type': 'ir.actions.report.xml',
                    'report_name': 'Request Tracking PCO',
                    'datas': data}
        else :
            context['landscape'] = True
            return self.pool['report'].get_action(
                cr, uid, [],
                'dym_report_request_trackingpco.report_request_trackingpco',
