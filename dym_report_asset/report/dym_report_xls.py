...
from .dym_report import dym_report_account_asset_print
...
_ir_translation_name = 'report.account.asset'


class dym_report_account_asset_print_xls(dym_report_account_asset_print):
    def __init__(self, cr, uid, name, context):
        super(dym_report_account_asset_print_xls, self).__init__(
            cr, uid, name, context=context)
        move_line_obj = self.pool.get('account.asset.asset')
        self.context = context
        wl_overview = move_line_obj._report_xls_account_asset_fields(
            cr, uid, context)
...
class report_account_asset_xls(report_xls):
    def __init__(self, name, table, rml=False,
                 parser=False, header=True, store=False):
        super(report_account_asset_xls, self).__init__(
        ...
        self.col_specs_template_overview = {
            ...
            'branch_name': {
                'header': [1, 22, 'text', _render("_('Branch Location')")],
                'lines': [1, 0, 'text', _render("p['branch_name']")],
                'totals': [1, 22, 'text', None]},
            'asset_code': {
                'header': [1, 22, 'text', _render("_('Asset Code')")],
                'lines': [1, 0, 'text', _render("p['asset_code']")],
                'totals': [1, 22, 'text', None]},
            'category_name': {
                'header': [1, 22, 'text', _render("_('Asset Category')")],
                'lines': [1, 0, 'text', _render("p['category_name']")],
                'totals': [1, 22, 'text', None]},
            'asset_account': {
                'header': [1, 22, 'text', _render("_('Asset Account')")],
                'lines': [1, 0, 'text', _render("p['asset_account']")],
                'totals': [1, 22, 'text', None]},
            'asset_name': {
                'header': [1, 22, 'text', _render("_('Asset Name')")],
                'lines': [1, 0, 'text', _render("p['asset_name']")],
                'totals': [1, 22, 'text', None]},
            'asset_owner': {
                'header': [1, 22, 'text', _render("_('Asset Owner')")],
            ...
            'purchase_date': {
                ...
                'totals': [1, 22, 'text', None]},
            'number_of_depr': {
                'header': [1, 22, 'text', _render("_('Number of Depr')")],
            ...
            'coa_depr_code': {
                'header': [1, 22, 'text', _render("_('COA Depr No')")],
                'lines': [1, 0, 'text', _render("p['coa_depr_code']")],
                'totals': [1, 22, 'text', None]},
            'coa_depr_name': {
                'header': [1, 22, 'text', _render("_('COA Depr Name')")],
            ...
            'status': {
                ...
                'totals': [1, 22, 'text', None]},
            'purchase_value': {
                'header': [1, 22, 'text', _render("_('Purchase Value')")],
            ...
            'depreciated_value': {
                'header': [1, 22, 'text', _render("_('Depreciated Value')")],
                'lines': [1, 0, 'number', _render("p['purchase_value'] - p['nbv']"), None, self.pd_cell_style_decimal],
                'totals': [1, 22, 'number', _render("p['purchase_value'] - p['nbv']"), None, self.rt_cell_style_decimal]},
            'nbv': {
            ...
            'depr_end_date': {
                'header': [1, 22, 'text', _render("_('Depreciation End Date')")],
                'lines': [1, 0, 'text', _render("p['depr_end_date']")],
                'totals': [1, 22, 'text', None]},
            'degressif_factor': {
                'header': [1, 22, 'text', _render("_('Degressive Factor')")],
                'lines': [1, 0, 'number', _render("p['degressif_factor']"), None, self.pd_cell_style_decimal],
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            report_name = ' '.join(
                [_('LAPORAN Asset Per Tanggal'), _(str(datetime.today().date())),
                 _p.report_info])
            c_specs_o = [
                ('report_name', 1, 0, 'text', report_name),
            ]
            ...
            ws_o.write(row_pos_o, 12, xlwt.Formula("SUM(M" + str(row_data_begin) + ":M" + str(row_data_end) + ")"),
                       self.rt_cell_style_decimal)
...
report_account_asset_xls('report.Laporan Asset', 'account.asset.asset', parser=dym_report_account_asset_print_xls)
