...
class dym_report_loan_pinjaman_print(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        if context is None:
            context = {}
        super(dym_report_loan_pinjaman_print, self).__init__(
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        report_loan_pinjaman = {
            'type': 'payable',
            'title': '',
            'title_short': title_short_prefix + ', ' + _('Laporan Loan Pinjaman')}
        ...
        query_start = "SELECT loan.id as id_ai, " \
            ...
            "loan.tipe_pinjaman as tipe_pinjaman, " \
            "loan.memo as memo, " \
            ...
            "where 1=1 and loan.state in ('approved','done') and loan.loan_type = 'Pinjaman' "
        ...
        if tipe_pinjaman :
            query_end +=" AND loan.tipe_pinjaman = '%s'" % str(tipe_pinjaman)
        if trx_start_date :
...
#                              'b': 3})
        ...
        if not reports :
            reports = [{'title_short': 'Laporan Loan Pinjaman', 'type': ['out_invoice','in_invoice','in_refund','out_refund'], 'id_ai':
                            [{'total': 0,
                              'date': 'NO DATA FOUND',
        ...
        super(dym_report_loan_pinjaman_print, self).set_context(
...
class wrapped_vat_declaration_print(orm.AbstractModel):
    _name = 'report.dym_report_loan_pinjaman.report_loan_pinjaman'
    _inherit = 'report.abstract_report'
    _template = 'dym_report_loan_pinjaman.report_loan_pinjaman'
    _wrapped_report_class = dym_report_loan_pinjaman_print
