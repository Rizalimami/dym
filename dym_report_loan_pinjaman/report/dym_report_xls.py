...
from .dym_report import dym_report_loan_pinjaman_print
from openerp.tools.translate import translate
import logging
_logger = logging.getLogger(__name__)
import string

_ir_translation_name = 'report.loan.pinjaman'

class dym_report_loan_pinjaman_print_xls(dym_report_loan_pinjaman_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_loan_pinjaman_print_xls, self).__init__(
            cr, uid, name, context=context)
        move_line_obj = self.pool.get('dym.loan')
        self.context = context
        wl_overview = move_line_obj._report_xls_loan_pinjaman_fields(
            cr, uid, context)
...
class report_loan_pinjaman_xls(report_xls):

    def __init__(self, name, table, rml=False,
                 parser=False, header=True, store=False):
        super(report_loan_pinjaman_xls, self).__init__(
        ...
        self.col_specs_template_overview = {
           ...
           'tipe_pinjaman': {
               'header': [1, 22, 'text', _render("_('Tipe Pinjaman')")],
               'lines': [1, 0, 'text', _render("p['tipe_pinjaman']")],
               'totals': [1, 22, 'text', None]},
           'rv_pv': {
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            report_name = ' '.join(
                [_('LAPORAN Loan Pinjaman Per Tanggal'), _(str(datetime.today().date())),
                 _p.report_info])
            c_specs_o = [
                ('report_name', 1, 0, 'text', report_name),
            ]
...
report_loan_pinjaman_xls('report.Laporan Loan Pinjaman', 'dym.loan', parser = dym_report_loan_pinjaman_print_xls)
