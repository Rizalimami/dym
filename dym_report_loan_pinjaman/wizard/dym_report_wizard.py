...
class dym_report_loan_pinjaman_wizard(orm.TransientModel):
    _name = 'dym.report.loan.pinjaman.wizard'
    _description = 'Report Loan Pinjaman Wizard'

    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if not context: context = {}
        res = super(dym_report_loan_pinjaman_wizard, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = {
        ...
        'tipe_pinjaman': fields.selection([('pendek','Jangka Pendek'),('panjang','Jangka Panjang')], 'Tipe Pinjaman', change_default=True, select=True),
        ...
        'branch_ids': fields.many2many('dym.branch', 'dym_report_loan_pinjaman_branch_rel', 'dym_report_loan_pinjaman_wizard_id',
                                        'branch_id', 'Branch', copy=False),
        'partner_ids': fields.many2many('res.partner', 'dym_report_loan_pinjaman_partner_rel', 'dym_report_loan_pinjaman_wizard_id',
                                        'partner_id', 'Partner', copy=False, domain=[('creditur_debitur','=',True)]),
        'account_ids': fields.many2many('account.account', 'dym_report_loan_pinjaman_account_rel', 'dym_report_loan_pinjaman_wizard_id',
...
#             journal_ids = self.pool.get('account.journal').search(cr, uid, [('type','in',['purchase','sale_refund'])])
        ...
        tipe_pinjaman = data['tipe_pinjaman']
        ...
        if context.get('xls_export'):
            return {'type': 'ir.actions.report.xml',
                    'report_name': 'Laporan Loan Pinjaman',
                    'datas': data}
        else :
            context['landscape'] = True
            return self.pool['report'].get_action(
                cr, uid, [],
                'dym_report_loan_pinjaman.report_loan_pinjaman',
