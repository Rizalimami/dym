...
class dym_report_other_receivable_wizard(orm.TransientModel):
    _name = 'dym.report.other.receivable.wizard'
    _description = 'Report Other Receivable Wizard'

    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if not context: context = {}
        res = super(dym_report_other_receivable_wizard, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = {
        ...
        'branch_ids': fields.many2many('dym.branch', 'dym_report_other_receivable_branch_rel', 'dym_report_other_receivable_wizard_id',
                                        'branch_id', 'Branch', copy=False),
        'partner_ids': fields.many2many('res.partner', 'dym_report_other_receivable_partner_rel', 'dym_report_other_receivable_wizard_id',
                                        'partner_id', 'Customer', copy=False, domain=[('customer','=',True)]),
        'account_ids': fields.many2many('account.account', 'dym_report_other_receivable_account_rel', 'dym_report_other_receivable_wizard_id',
                                        'account_id', 'Account', copy=False, domain=[('type','=','receivable')]),
        'journal_ids': fields.many2many('account.journal', 'dym_report_other_receivable_journal_rel', 'dym_report_other_receivable_wizard_id',
                                        'journal_id', 'Journal', copy=False, domain=[('type','in',['sale','sale_refund'])]),
...
#             journal_ids = self.pool.get('account.journal').search(cr, uid, [('type','in',['purchase','sale_refund'])])
        ...
        if context.get('xls_export'):
            return {'type': 'ir.actions.report.xml',
                    'report_name': 'Laporan Other Receivable',
                    'datas': data}
        else :
            context['landscape'] = True
            return self.pool['report'].get_action(
                cr, uid, [],
                'dym_report_other_receivable.report_other_receivable',
