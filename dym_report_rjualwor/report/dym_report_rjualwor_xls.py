...
from .dym_report_rjualwor import dym_report_rjualwor_print
from openerp.tools.translate import translate
import string

class dym_report_rjualwor_print_xls(dym_report_rjualwor_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_rjualwor_print_xls, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'datetime': datetime,
            'wanted_list_overview': self.pool.get('dym.work.order')._report_xls_rjualwor_fields(cr, uid, context),
            '_': self._,
        })

    def _(self, src):
        lang = self.context.get('lang', 'en_US') 
        return translate(self.cr, 'report.rjualwor', 'report', lang, src) or src

class report_rjualwor_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_rjualwor_xls, self).__init__(name, table, rml, parser, header, store)
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ws_o = wb.add_sheet('Laporan Jual WOR')
            ...
            c_specs_o = [('report_name', 1, 0, 'text', 'Laporan Jual WOR')]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(ws_o, row_pos_o, row_data, row_style=cell_style)
...
report_rjualwor_xls('report.Laporan Jual WOR', 'dym.work.order', parser = dym_report_rjualwor_print_xls)
