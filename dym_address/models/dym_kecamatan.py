...
class dym_kecamatan(osv.osv):
    ...
    def get_province_by_city(self, cr, uid, ids, city_id):
        if city_id == city_id:
            city= self.pool.get("dym.city").browse(cr, uid, city_id)
    ...
    _columns = {
        ...
        'kelurahan_ids': fields.one2many('dym.kelurahan', 'kecamatan_id', string='Kelurahan', readonly=True),
    }
    
    _sql_constraints = [
       ('code_unique', 'unique(code,city_id)', 'Code Kecamatan tidak boleh ada yang sama.'),  
    ]
