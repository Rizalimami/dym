...
from .dym_report_kinerjasa import dym_report_kinerjasa_print
from openerp.tools.translate import translate
import string

class dym_report_kinerjasa_print_xls(dym_report_kinerjasa_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_kinerjasa_print_xls, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'datetime': datetime,
            'wanted_list_overview': self.pool.get('dym.work.order')._report_xls_kinerjasa_fields(cr, uid, context),
            '_': self._,
        })

    def _(self, src):
        lang = self.context.get('lang', 'en_US') 
        return translate(self.cr, 'report.kinerjasa', 'report', lang, src) or src

class report_kinerjasa_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_kinerjasa_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_overview = {
            ...
            'nama_sa': {
                'header': [1, 32, 'text', _render("_('Nama SA')")],
            ...
            'rp_accessories': {
                'header': [1, 15, 'text', _render("_('Rp. Accessories')")],
                'lines': [1, 0, 'number', _render("p['rp_accessories']"), None, self.pd_cell_style_decimal],
                'totals': [1, 15, 'number', _render("p['rp_accessories']"), None, self.pd_cell_style_decimal]},
        }
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ws_o = wb.add_sheet('Laporan Kinerja SA')
            ...
            c_specs_o = [('report_name', 1, 0, 'text', 'Laporan Prestasi/Kinerja Service Advisor')]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(ws_o, row_pos_o, row_data, row_style=cell_style)
...
report_kinerjasa_xls('report.Laporan Kinerja SA', 'dym.work.order', parser = dym_report_kinerjasa_print_xls)
