...
from .dym_report_stock_movement import dym_report_stock_movement_print
from openerp.tools.translate import translate
import string

_ir_translation_name = 'report.stock.movement'

class dym_report_stock_movement_print_xls(dym_report_stock_movement_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_stock_movement_print_xls, self).__init__(cr, uid, name, context=context)
        move_obj = self.pool.get('stock.move')
        self.context = context
        wl_overview = move_obj._report_xls_stock_movement_fields(cr, uid, context)
        #aris
        wl_overview_sparepart = move_obj._report_xls_stock_movement_fields_sparepart(cr,uid, context)
...
class report_stock_movement_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_stock_movement_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_overview = {
            ...
            'picking_type_name': {
                ...
                'totals': [1, 22, 'text', None]},
            'packing_name': {
                'header': [1, 22, 'text', _render("_('Movement Number')")],
            ...
            'packing_state': {
                ...
                'totals': [1, 22, 'text', None]},
            'packing_date': {
                'header': [1, 22, 'text', _render("_('Movement Date')")],
            ...
            'location': {
                'header': [1, 22, 'text', _render("_('Location Tujuan (Child)')")],
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            division = r['division']
            hutang_piutang_ksu = r['hutang_piutang_ksu']
            ...
            if hutang_piutang_ksu == 'hutang':
                c_specs_o = [
                    ('report_name', 1, 0, 'text', 'LAPORAN Hutang KSU'),
                ]
            elif hutang_piutang_ksu == 'piutang':
                c_specs_o = [
                    ('report_name', 1, 0, 'text', 'LAPORAN Piutang KSU'),
                ]
            else:
                c_specs_o = [
                    ('report_name', 1, 0, 'text', 'LAPORAN STOCK MOVEMENT'),
                ]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(
                ws_o, row_pos_o, row_data, row_style=cell_style)
            ...
            if hutang_piutang_ksu:
                ws_o.write(row_pos_o + 2, 0, 'Per Tanggal ' + ('-' if data['date_start_date'] == False else str(data['date_start_date'])))
            else:
                ...
                ws_o.write(row_pos_o + 3, 0, 'Scheduled Date ' + ('-' if data['min_date_start_date'] == False else str(data['min_date_start_date'])) + ' s/d ' + ('-' if data['min_date_end_date'] == False else str(data['min_date_end_date'])))
                ws_o.write(row_pos_o + 4, 0, 'Date of Transfer ' + ('-' if data['date_done_start_date'] == False else str(data['date_done_start_date'])) + ' s/d ' + ('-' if data['date_done_end_date'] == False else str(data['date_done_end_date'])))

report_stock_movement_xls(
    'report.Laporan Stock Movement',
    'stock.move',
    parser = dym_report_stock_movement_print_xls)
