...
class dym_kinerja_mekanik_report(osv.osv_memory):

    _name = 'dym.kinerja.mekanik.report'
    _description = 'Laporan Kinerja Mekanik'
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if not context:
            context = {}
        res = super(dym_kinerja_mekanik_report, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = {
        'branch_ids': fields.many2many('dym.branch', 'dym_report_mekanik_branch_rel', 'dym_report_mekanik_wizard_id','branch_id', 'Branches', copy=False),
        'mekanik_ids': fields.many2many('hr.employee', 'dym_report_mekanik_user_rel', 'dym_report_mekanik_wizard_id','mekanik_id', 'Mekanik', copy=False),
        'product_ids': fields.many2many('product.product', 'dym_report_mekanik_product_rel', 'dym_report_mekanik_wizard_id','product_id', 'Products'),
    ...
    def onchange_branch(self, cr, uid, ids, branchs, context=None):
        ...
        domain = {}
        warning = {}
        value['mekanik_ids'] = False
        if branchs[0][2]:
            mekanik_ids = self.pool.get('hr.employee').search(cr, uid, [('branch_id','in',branchs[0][2]),('job_id.mekanik','=',True)])
            domain['mekanik_ids'] = [('id','in',mekanik_ids)]
        else:
            branch_ids = self._get_branch_ids(cr, uid, context)
            mekanik_ids = self.pool.get('hr.employee').search(cr, uid, [('branch_id','in',branch_ids),('job_id.mekanik','=',True)])
            domain['mekanik_ids'] = [('id','in',mekanik_ids)]
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        return {'type': 'ir.actions.report.xml', 'report_name': 'Laporan Kinerja Mekanik', 'datas': data}
