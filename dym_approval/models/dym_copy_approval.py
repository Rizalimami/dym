...
class dym_copy_approval(models.TransientModel):
    _name = "dym.copy.approval"
    _description = "Copy Approval"
    _rec_name = "approval_type"
    
    def _get_form(self):
        form_ids = self.env['dym.approval.config'].search([], order='name')
        selection = [('all','All')]
    ...
    division = fields.Selection([('all','All'),('Unit','Showroom'),('Sparepart','Workshop'),('Umum','General'),('Finance','Finance')], string='Division')
    form_id = fields.Selection(_get_form, string='Form')
    branch_from_id = fields.Many2one('dym.branch', string='Branch From')
    branch_to_id = fields.Many2one('dym.branch', string='Branch To')
    ...
    def create_approval_matrixbiaya(self, biaya_header_ids, id_branch_from, id_branch_to):
        if biaya_header_ids :
            obj_biaya_header = self.env['dym.approval.matrixbiaya.header']
            obj_biaya = self.env['dym.approval.matrixbiaya']
            ...
            raise osv.except_osv("Perhatian", "Tidak ditemukan Approval Matrix Biaya '%s' untuk Form dan Division yg dipilih !" %self.env['dym.branch'].browse(id_branch_from).name)
    
    @api.multi
    def create_approval_matrixdiscount(self, disc_header_ids, id_branch_from, id_branch_to):
        if disc_header_ids :
            obj_disc_header = self.env['dym.approval.matrixdiscount.header']
            obj_disc = self.env['dym.approval.matrixdiscount']
            for disc_header_id in disc_header_ids :
                ...
                new_disc_id = disc_header_id.copy({'branch_id':self.branch_to_id.id, 'dym_approval_md_ids':[]})
                disc_id = obj_disc.search([('dym_approval_md_id','=',disc_header_id.id)])
                disc_id.copy({'dym_approval_md_id': new_disc_id.id, 'branch_id':id_branch_to})
        else :
            raise osv.except_osv("Perhatian", "Tidak ditemukan Approval Matrix Discount '%s' untuk Form dan Division yg dipilih !" %self.env['dym.branch'].browse(id_branch_from).name)
