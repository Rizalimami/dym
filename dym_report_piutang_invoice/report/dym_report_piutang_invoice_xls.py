...
from .dym_report_piutang_invoice import dym_report_piutang_invoice_print
from openerp.tools.translate import translate
import logging
_logger = logging.getLogger(__name__)
import string

_ir_translation_name = 'report.piutang.invoice'

class dym_report_piutang_invoice_print_xls(dym_report_piutang_invoice_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_piutang_invoice_print_xls, self).__init__(
            cr, uid, name, context=context)
        move_line_obj = self.pool.get('account.invoice')
        self.context = context
        wl_overview = move_line_obj._report_xls_piutang_invoice_fields(
            cr, uid, context)
...
class report_piutang_invoice_xls(report_xls):

    def __init__(self, name, table, rml=False,
                 parser=False, header=True, store=False):
        super(report_piutang_invoice_xls, self).__init__(
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            report_name = ' '.join(
                [_('LAPORAN Piutang Per Tanggal'), _(str(datetime.today().date())),
                 _p.report_info])
            c_specs_o = [
                ('report_name', 1, 0, 'text', report_name),
            ]
...
report_piutang_invoice_xls('report.Laporan Piutang Invoice', 'account.invoice', parser = dym_report_piutang_invoice_print_xls)
