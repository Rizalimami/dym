...
class dym_report_piutang_invoice_print(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        if context is None:
            context = {}
        super(dym_report_piutang_invoice_print, self).__init__(
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        report_piutang_invoice = {
            'type': 'payable',
            'title': '',
            'title_short': title_short_prefix + ', ' + _('Laporan Piutang Invoice')}
        ...
        query_start = "SELECT ai.id as id_ai, " \
            ...
            "where 1=1 and ai.type in ('out_invoice','in_refund') "
        ...
        reports = [report_piutang_invoice]
                ...
                for p in p_map:
                   if p['id_ai'] not in map(
                       ...
                       if analytic:
                           ...
                           # move_line = ai.move_id.line_id.filtered(lambda r: r.account_id.id == ai.account_id.id and r.name == name)
                           # if len(move_line) > 1:
                           #    move_line = move_line.filtered(lambda r: r.debit == ai.amount_total or r.debit == ai.amount_total - ai.amount_dp)
                           # if not move_line:
                           #    move_line = ai.move_id.line_id.filtered(lambda r: r.debit == ai.amount_total or r.debit == ai.amount_total - ai.amount_dp and r.name == name)
                           # if len(move_line) > 1:
                           #    move_line = move_line.sorted(key=lambda r: r.id)[0]
                           # residual = self.pool.get('account.move.line').get_residual_date_based(cr, uid, move_line.id, trx_end_date)
...
#                              'b': 3})
        ...
        if not reports :
            reports = [{'title_short': 'Laporan Piutang Invoice', 'type': ['out_invoice','in_invoice','in_refund','out_refund'], 'id_ai':
                            [{'total': 0,
        ...
        super(dym_report_piutang_invoice_print, self).set_context(
...
class wrapped_vat_declaration_print(orm.AbstractModel):
    _name = 'report.dym_report_piutang_invoice.report_piutang_invoice'
    _inherit = 'report.abstract_report'
    _template = 'dym_report_piutang_invoice.report_piutang_invoice'
    _wrapped_report_class = dym_report_piutang_invoice_print
