...
class dym_unit_bundling(models.Model):
    _name = "dym.unit.bundling"
    ...
    bundling_line = fields.One2many('dym.unit.bundling.line', 'bundling_id', 'Bundling Line')
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        vals['name'] = self.pool.get('ir.sequence').get_per_branch(cr, uid, vals['branch_id'], 'UB', division=False)
        unit_bundling = super(dym_unit_bundling, self).create(cr, uid, vals, context=context)
    ...
    def confirm(self):
        ...
        return self.write({'state': 'done','confirm_uid':self._uid,'confirm_date':datetime.now(),'date':datetime.today()})
...
class dym_unit_bundling_line(models.Model):
    _name = "dym.unit.bundling.line"
    _description = "Unit Bundling Line"

    bundling_id = fields.Many2one('dym.unit.bundling',string='Bundling')
...
class dym_reject_approval_unit_bundling(models.TransientModel):
    _name = "dym.reject.approval.unit.bundling"
    ...
    def dym_reject_approval(self, context=None):
        ...
        line = self.env['dym.unit_bundling'].browse(unit_bunlind_id,context=context)
        objek = False
                    ...
                    if y == x.group_id :
                        ...
                        for z in line.approval_ids :
                            if z.reason == False :
                                ...
                                self.env['dym.unit_bundling'].write({'state':'confirm','approval_state':'r'})
        if objek == False :
            raise exceptions.ValidationError("User tidak termasuk group approval")
