...
class dealer_pengembalian_spk(models.Model):
    ...
    _description = "Pengembalian Memo Dealer"
    _order = "id asc"
    ...
    def unlink(self, cr, uid, ids, context=None):
        for item in self.browse(cr, uid, ids, context=context):
            if item.state != 'draft':
                raise osv.except_osv(('Perhatian !'), ("Dealer Pengembalian Memo sudah diproses, data tidak bisa didelete !"))
...
class dealer_pengembalian_spk_line(models.Model):
    ...
    _sql_constraints = [
    ('unique_dealer_register_spk_line', 'unique(dealer_register_spk_line_id,pengembalian_spk_id)', 'Nomor register duplicate!'),
    ]
...
class dealer_reason_pengembalian_spk_cancel(models.TransientModel):
    ...
    def action_post_cancel(self, context=None):
        ...
        for pengembalian_line in pengembalian_obj.pengembalian_spk_ids:
            if pengembalian_line.register_state != 'open':
                pengembalian_line.dealer_register_spk_line_id.write({'state':pengembalian_line.register_state,'reason_kembali':self.reason})
            else:
                pengembalian_line.dealer_register_spk_line_id.write({'state':pengembalian_line.register_state,'sales_id':False,'tanggal_distribusi':False,'reason_kembali':self.reason})
