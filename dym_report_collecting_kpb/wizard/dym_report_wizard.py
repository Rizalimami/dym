...
class dym_report_collecting_kpb_wizard(orm.TransientModel):
    _name = 'dym.report.collecting.kpb.wizard'
    _description = 'Report Collecting KPB Wizard'

    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if not context: context = {}
        res = super(dym_report_collecting_kpb_wizard, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = {
        ...
        'branch_ids': fields.many2many('dym.branch', 'dym_report_collecting_kpb_branch_rel', 'dym_report_collecting_kpb_wizard_id',
                                        'branch_id', 'Branch', copy=False),
        'partner_ids': fields.many2many('res.partner', 'dym_report_collecting_kpb_partner_rel', 'dym_report_collecting_kpb_wizard_id',
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        if context.get('xls_export'):
            return {'type': 'ir.actions.report.xml',
                    'report_name': 'Laporan Collecting KPB',
                    'datas': data}
        else :
            context['landscape'] = True
            return self.pool['report'].get_action(
                cr, uid, [],
                'dym_report_collecting_kpb.report_collecting_kpb',
