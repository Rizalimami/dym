...
from .dym_report import dym_report_loan_piutang_print
from openerp.tools.translate import translate
import logging
_logger = logging.getLogger(__name__)
import string

_ir_translation_name = 'report.loan.piutang'

class dym_report_loan_piutang_print_xls(dym_report_loan_piutang_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_loan_piutang_print_xls, self).__init__(
            cr, uid, name, context=context)
        move_line_obj = self.pool.get('dym.loan')
        self.context = context
        wl_overview = move_line_obj._report_xls_loan_piutang_fields(
            cr, uid, context)
...
class report_loan_piutang_xls(report_xls):

    def __init__(self, name, table, rml=False,
                 parser=False, header=True, store=False):
        super(report_loan_piutang_xls, self).__init__(
        ...
        self.col_specs_template_overview = {
            ...
            'number': {
                'header': [1, 22, 'text', _render("_('Number Loan')")],
           ...
           'ref': {
            ...
            'date_due': {
                ...
                'totals': [1, 22, 'text', None]},
            'effective_date': {
                'header': [1, 22, 'text', _render("_('Tgl Efektif')")],
           ...
           'state': {
               ...
               'totals': [1, 22, 'text', None]},
            'total': {
                'header': [1, 22, 'text', _render("_('Total Loan Piutang')")],
            ...
            'residual': {
                'header': [1, 22, 'text', _render("_('Saldo Piutang')")],
           ...
           'rv_pv': {
               'header': [1, 22, 'text', _render("_('PV')")],
               'lines': [1, 0, 'text', _render("p['rv_pv']")],
               'totals': [1, 22, 'text', None]},
           'loan_type': {
               'header': [1, 22, 'text', _render("_('Tipe Loan')")],
               'lines': [1, 0, 'text', _render("p['loan_type']")],
               'totals': [1, 22, 'text', None]},
        }
        # XLS Template
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            report_name = ' '.join(
                [_('LAPORAN Loan Piutang Per Tanggal'), _(str(datetime.today().date())),
                 _p.report_info])
            c_specs_o = [
                ('report_name', 1, 0, 'text', report_name),
            ]
...
report_loan_piutang_xls('report.Laporan Loan Piutang', 'dym.loan', parser = dym_report_loan_piutang_print_xls)
