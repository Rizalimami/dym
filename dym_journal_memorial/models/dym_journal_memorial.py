...
class dym_account_move(models.Model):
    ...
    partner_type = fields.Selection([('Pihak_ke_3','Pihak Ke 3'),('Afiliasi','Afiliasi'),('Konsolidasi','Konsolidasi')], string='Tipe Partner')

class dym_account_move(models.Model):
    _inherit = 'account.move'
     
    reverse_from_id = fields.Many2one('account.move', string='Reversed From', copy=False)

class dym_journal_memorial(models.Model):
    _name = 'dym.journal.memorial'
    ...
    journal_memorial_line = fields.One2many('dym.journal.memorial.line','journal_memorial_id')
    ...
    cancel_refered = fields.Many2one('dym.journal.memorial')
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        res = super(dym_journal_memorial, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
        periode_obj = self.pool.get('account.period')
        kolek_periode =[]
        user_obj = self.pool.get('res.users').browse(cr, uid, uid)
        periode_now = periode_obj.search(cr,uid,[
    ...
    def create(self,vals,context=None):
        ...
        vals['name'] = self.env['ir.sequence'].get_per_branch(vals['branch_id'], 'JM', division=False) 
        ...
        res =  super(dym_journal_memorial, self).create(vals)  
        res.cek_balance()
        return res

    @api.multi
    def write(self,values,context=None):
        res =  super(dym_journal_memorial,self).write(values)
        self.cek_balance()
        return res
    ...
    def action_create_memorial(self):
        ...
        for line in self.journal_memorial_line :
            ...
            self.env['dym.journal.memorial.line'].sudo().create(memorial_line_vals)
    ...
    def create_intercompany_lines(self,cr,uid,ids,move_id,context=None):
        ...
        branch_config_journal = self.pool.get('dym.branch.config').search(cr,uid,[
                                           ('branch_id','=',vals.branch_id.id),
                                           ('journal_memorial_journal_id','!=',False)
                                           ])
        if not branch_config_journal :
            raise osv.except_osv(('Perhatian !'), ("Journal Memorial belum diisi dalam master Branch Config !"))  
        branch_journal = self.pool.get('dym.branch.config').browse(cr,uid,branch_config_journal)
        journal_id = branch_journal.journal_memorial_journal_id.id   
    ...
    def wkf_request_approval(self):
        ...
        branch_config_journal = self.env['dym.branch.config'].search([
                                           ('branch_id','=',self.branch_id.id),
                                           ('journal_memorial_journal_id','!=',False)
                                           ])
        if not branch_config_journal :
    ...
    def view_jm(self,cr,uid,ids,context=None):  
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'dym.journal.memorial',
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(dym_journal_memorial, self).unlink(cr, uid, ids, context=context) 
                                
class dym_journal_memorial_line(models.Model):
    _name = 'dym.journal.memorial.line'
    ...
    def _get_analytic_company(self):
        ...
        if not level_1_ids:
            raise osv.except_osv(('Perhatian !'), ("[dym_journal_memorial] Tidak ditemukan data analytic untuk company %s")%(company.name))
    ...
    journal_memorial_id = fields.Many2one('dym.journal.memorial')
