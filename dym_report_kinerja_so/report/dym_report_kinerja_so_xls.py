...
from .dym_report_kinerja_so import dym_report_kinerja_so_print
from openerp.tools.translate import translate
import string

_ir_translation_name = 'report.kinerja.so'

class dym_report_kinerja_so_print_xls(dym_report_kinerja_so_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_kinerja_so_print_xls, self).__init__(cr, uid, name, context=context)
        kw_obj = self.pool.get('sale.order')
        self.context = context
        wl_overview = kw_obj._report_xls_kinerja_so_fields(cr, uid, context)
...
class report_kinerja_so_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_kinerja_so_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_overview = {
            ...
            'so_name': {
                ...
                'lines': [1, 0, 'text', _render("p['so_name']"), None, self.pd_cell_style_decimal],
                'totals': [1, 22, 'text', _render("p['so_name']")]},
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            c_specs_o = [
                ('report_name', 1, 0, 'text', 'LAPORAN KINERJA SO'),
            ]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(
                ws_o, row_pos_o, row_data, row_style=cell_style)
...
report_kinerja_so_xls(
    'report.Laporan Kinerja SO',
    'sale.order',
    parser = dym_report_kinerja_so_print_xls)
