...
class dym_purchase_order(osv.osv):
    ...
    def wkf_request_approval(self, cr, uid, ids, context=None):
        ...
        if branch_id and division:
            name = self.pool.get('ir.sequence').get_per_branch(cr, uid, branch_id, 'POR', division=division)
            self.write(cr,uid,ids,{'name':name},context=context)

        code = obj_po.purchase_order_type_id.name
        code = code.lower()
        code = code.replace(' ','_')
        code = code.replace('  ','_')
        code = code.replace('   ','_')
        obj_matrix.request(cr, uid, ids, obj_po, 'amount_total', code)
