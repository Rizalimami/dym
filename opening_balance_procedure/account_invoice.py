...
class account_invoice(osv.osv):
    ...
    def bulk_invoice(self, cr, uid, data, context=None):
        result = []
        invoices = []
        invoice_lines = []
        moves = []
	    ...
	    cr.execute("""SELECT id FROM ir_model_data WHERE name = %s""", ((dt['Partner'],)))
            partner_id = int(cr.fetchone()[0])
        ...
        cr.executemany("""INSERT INTO account_invoice_line 
            (create_uid,create_date,write_date,write_uid,name,account_id,invoice_id,analytic_1,analytic_2,analytic_3,account_analytic_id,company_id,price_unit,quantity,price_subtotal)
