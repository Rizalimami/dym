...
class account_invoice(osv.osv):
    ...
    def bulk_invoice_ap(self, cr, uid, data, context=None):
        result = []
        invoices = []
        invoice_lines = []
        moves = []
        ...
        # Check data integroty before processing
        # for n,dt in enumerate(data,start=0):
        #     module_analytic = dt['Analytic Account'].split('.',1)[0]
        #     cr.execute("""SELECT id FROM account_analytic_account WHERE   id = %s """, (analytic_4))
