...
from .dym_report_workshop import dym_report_workshop_print
from openerp.tools.translate import translate
import string

class dym_report_workshop_print_xls(dym_report_workshop_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_workshop_print_xls, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'datetime': datetime,
            'wanted_list_overview': self.pool.get('dym.work.order')._report_xls_workshop_fields(cr, uid, context),
            '_': self._,
        })
...
class report_workshop_xls(report_xls):
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            # Start Date & End Date
            cell_style = xlwt.easyxf(_xs['left'])
            report_name = ' '.join([_('Tanggal'), _('-' if data['start_date'] == False else str(data['end_date'])), _('s/d'), _('-' if data['end_date'] == False else str(data['end_date']))])
...
report_workshop_xls('report.Laporan Workshop', 'dym.work.order', parser = dym_report_workshop_print_xls)
