...
class dym_report_workshop_print(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        if context is None:
            context = {}
        super(dym_report_workshop_print, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        query_workshop = """
            ...
            '' as login, mechanic.name_related as mechanic, lot.no_polisi as nopol, customer.default_code as cust_code, customer.name as cust_name, customer.mobile as cust_mobile, unit.name_template as unit_name, lot.name as engine, lot.chassis_no as chassis, wol.categ_id as wo_categ, prod_category.name as prod_categ_name, 
            CASE 
                WHEN db.id is not null THEN prod_category2.name
                ELSE prod_category.name 
            END as prod_categ_name, 
            CASE 
                WHEN db.id is not null THEN product2.name_template
                ELSE product.name_template 
            END as prod_name, 
            CASE 
                WHEN db.id is not null THEN product2.default_code
                ELSE product.default_code 
            END as  prod_code, 
            CASE 
                WHEN wol.categ_id = 'Sparepart' THEN wol.supply_qty
                WHEN db.location_id is not null THEN db.product_uom_qty
                ELSE wol.product_qty 
            END as qty, 
            CASE 
                WHEN db.id is not null THEN db.price_bundle
                ELSE wol.price_unit 
            END as het, 
            CASE 
                WHEN db.id is not null THEN ail2.force_cogs / COALESCE(NULLIF(ail2.quantity,0),1) * db.product_uom_qty
                ELSE ail.force_cogs / COALESCE(NULLIF(ail.quantity,0),1) * wol.supply_qty  
            END as hpp, 
            CASE 
                WHEN db.id is not null THEN db.diskon+wol.discount
                ELSE wol.discount 
            END as discount, 
            CASE 
                WHEN wol.categ_id = 'Sparepart' THEN wol.price_unit * (1 - wol.discount / 100) / 1.1 * wol.supply_qty
                WHEN db.location_id is not null THEN (db.price_bundle * (1 - wol.discount / 100)-db.diskon) / 1.1 * db.product_uom_qty
                WHEN db.location_id is null and db.id is not null THEN (db.price_bundle * (1 - wol.discount / 100)-db.diskon) / 1.1 * db.product_uom_qty
                ELSE wol.price_unit * (1 - wol.discount / 100) / 1.1 * wol.product_qty  
            END as dpp, 
            CASE 
                WHEN wol.categ_id = 'Sparepart' THEN wol.price_unit * (1 - wol.discount / 100) / 1.1 * 0.1 * wol.supply_qty
                WHEN db.location_id is not null THEN (db.price_bundle * (1 - wol.discount / 100)-db.diskon) / 1.1 * 0.1 * db.product_uom_qty
                WHEN db.location_id is null and db.id is not null THEN (db.price_bundle * (1 - wol.discount / 100)-db.diskon) / 1.1 * 0.1 * db.product_uom_qty
                ELSE wol.price_unit * (1 - wol.discount / 100) / 1.1 * 0.1 * wol.product_qty  
            END as ppn, 
            CASE 
                WHEN wol.categ_id = 'Sparepart' THEN wol.price_unit * (1 - wol.discount / 100) * wol.supply_qty
                WHEN db.location_id is not null THEN (db.price_bundle * (1 - wol.discount / 100)-db.diskon) * db.product_uom_qty
                WHEN db.location_id is null and db.id is not null THEN (db.price_bundle * (1 - wol.discount / 100)-db.diskon) * db.product_uom_qty
                ELSE wol.price_unit * (1 - wol.discount / 100) * wol.product_qty  
            END as total, 
            ...
            from dym_work_order wo inner join dym_work_order_line wol on wo.id = wol.work_order_id
            left join dym_branch b on wo.branch_id = b.id
            left join res_users users on users.id = wo.mekanik_id
            left join hr_employee mechanic on mechanic.id = wo.mekanik_id
            ...
            left join account_invoice_line ail on ail.origin = wo.name and ail.product_id = wol.product_id
            left join res_partner inv on b.default_supplier_workshop_id = inv.id
            left join dym_faktur_pajak_out fp ON wo.faktur_pajak_id = fp.id
            left join dym_work_order_bundle db ON db.wo_line_id = wol.id
            left join account_invoice_line ail2 on ail2.origin = wo.name and ail2.product_id = db.product_id
            left join product_product product2 on db.product_id = product2.id
            left join product_template prod_template2 on product2.product_tmpl_id = prod_template2.id
            left join product_category prod_category2 on prod_template2.categ_id = prod_category2.id
        """
        ...
        super(dym_report_workshop_print, self).set_context(objects, data, ids, report_type)
    ...
    def formatLang_zero2blank(self, value, digits=None, date=False, date_time=False, grouping=True, monetary=False, dp=False, currency_obj=False):
        if isinstance(value, (float, int)) and not value:
            ...
            return super(dym_report_workshop_print, self).formatLang(value, digits, date, date_time, grouping, monetary, dp, currency_obj)

class wrapped_vat_declaration_print(orm.AbstractModel):
    _name = 'report.dym_report_workshop.report_workshop'
    _inherit = 'report.abstract_report'
    _template = 'dym_report_workshop.report_workshop'
    _wrapped_report_class = dym_report_workshop_print
