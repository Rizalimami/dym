...
from .dym_report_rjualdwh import dym_report_rjualdwh_print
from openerp.tools.translate import translate
import string

class dym_report_rjualdwh_print_xls(dym_report_rjualdwh_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_rjualdwh_print_xls, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'datetime': datetime,
            'wanted_list_overview': self.pool.get('dealer.spk')._report_xls_rjualdwh_fields(cr, uid, context),
            '_': self._,
        })

    def _(self, src):
        lang = self.context.get('lang', 'en_US') 
        return translate(self.cr, 'report.rjualdwh', 'report', lang, src) or src

class report_rjualdwh_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_rjualdwh_xls, self).__init__(name, table, rml, parser, header, store)
...
report_rjualdwh_xls('report.Laporan Jual DWH', 'dealer.spk', parser = dym_report_rjualdwh_print_xls)
