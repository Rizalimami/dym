...
from .report_bank_book_new import dym_bank_book_new_report_print
from openerp.tools.translate import translate
import logging
_logger = logging.getLogger(__name__)
import time

_ir_translation_name = 'dym.bank.book.new'

class dym_bank_book_new_print_xls(dym_bank_book_new_report_print):

    def __init__(self, cr, uid, name, context):
        super(dym_bank_book_new_print_xls, self).__init__(
            cr, uid, name, context=context)
        moveline_obj = self.pool.get('account.move.line')
        self.context = context
        wl_overview = moveline_obj._report_xls_bank_book_new_fields(
            cr, uid, context)
...
class bank_book_new_report_xls(report_xls):

    def __init__(self, name, table, rml=False,
                 parser=False, header=True, store=False):
        super(bank_book_new_report_xls, self).__init__(
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            report_name = _(' '.join(['LAPORAN BANK BOOK NEW','(PROYEKSI)' if data['projection'] else '']))
            c_specs_o = [
...
bank_book_new_report_xls(
    'report.dym_report_bank_book_new_xls',
    'account.move.line',
    parser=dym_bank_book_new_print_xls)