...
class dym_pettycash_line(models.Model): 
    ...
    def default_get(self, fields):
        ...
        if branch_id:
            ...
            if division not in ['Umum']:
                analytic_1_general, analytic_2_general, analytic_3_general, analytic_4_general = self.pool.get('account.analytic.account').get_analytical(self._cr, self._uid, branch, division, False, 4, 'General')
    ...
    @api.onchange('account_id','analytic_4')
    def onchange_account_id(self):
        domain = {}
        values = {}
        branch_id = self._context.get('branch_id',[])
        division = self._context.get('division','General')
        journal_id = self._context.get('journal_id',False)
        aa_obj = self.env['account.analytic.account']


        if 'analytic_4' in domain:
            del domain['analytic_4']
        if 'analytic_4' in values:
            del values['analytic_4']
        if branch_id:
            branch = self.env['dym.branch'].browse([branch_id])
            aa1_ids, aa2_ids, aa3_ids, aa4_ids, df1, df2, df3, df4, aa_dict = self.env['analytic.account.filter'].get_analytics(branch_id, division, self.account_id.id)
            if all([aa1_ids, aa2_ids, aa3_ids, aa4_ids, df1, df2, df3, df4]):
                values['analytic_1'] = df1
                values['analytic_2'] = df2
                values['analytic_3'] = df3
                values['analytic_4'] = df4
                values['account_analytic_id'] = df4
                domain['analytic_1'] = [('id','in',aa1_ids)]
                domain['analytic_2'] = [('id','in',aa2_ids)]
                domain['analytic_3'] = [('id','in',aa3_ids)]
                domain['analytic_4'] = [('id','in',aa4_ids)]

                print "domain analytic_4==",[('id','in',aa4_ids)]

            kas_bon = self._context.get('kas_bon',False)
            if kas_bon:
                is_bon_sementara = self.env['account.account'].search([('is_bon_sementara','=',True)])
                if is_bon_sementara:
                    domain['account_id'] = [('id','in',is_bon_sementara.ids)]
                    values['account_id'] = is_bon_sementara[0].id
                else:
                    raise osv.except_osv(('Perhatian!'), ('System tidak menemukan akun yang ditandai sebagai Bon Sementara. Silahkan hubungi system administrator.'))
        return {'domain':domain,'value':values}

    @api.onchange('analytic_2')
    def onchange_analytic_2(self):
        if self.analytic_2:
            print "=======analytic_2===",self.analytic_2


    @api.onchange('account_id','name')
    def name_change(self):
        ...
        filter = self.env['dym.account.filter']
        if self.pettycash_id.kas_bon == True:
            dict = filter.get_domain_account("pettycash_kasbon")
        else:
    ...
    @api.onchange('amount')
    def amount_change(self):  
        if self.amount and self.pettycash_id.balance:
            if self.amount > self.pettycash_id.balance:
                self.amount = 0.0
                warning = {
                    'title': ('Perhatian !'),
                    'message': (("Nilai transaksi tidak boleh lebih dari saldo tersedia")),
                }
                return {'warning':warning} 
