...
from .dym_report_rjualacc import dym_report_rjualacc_print
from openerp.tools.translate import translate
import string

class dym_report_rjualacc_print_xls(dym_report_rjualacc_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_rjualacc_print_xls, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'datetime': datetime,
            'wanted_list_overview': self.pool.get('dealer.spk')._report_xls_rjualacc_fields(cr, uid, context),
            '_': self._,
        })

    def _(self, src):
        lang = self.context.get('lang', 'en_US') 
        return translate(self.cr, 'report.rjualacc', 'report', lang, src) or src

class report_rjualacc_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_rjualacc_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_overview = {
            ...
            'ps_ahm_real': {
                'header': [1, 22, 'text', _render("_('PS AHM (Real)')")],
            ...
            'ps_md_real': {
                'header': [1, 22, 'text', _render("_('PS MD (Real)')")],
            ...
            'ps_finco_real': {
                'header': [1, 22, 'text', _render("_('PS Finco (Real)')")],
            ...
            'ps_dealer_real': {
                'header': [1, 22, 'text', _render("_('PS Dealer (Real)')")],
            ...
            'ps_other_real': {
                'header': [1, 22, 'text', _render("_('PS Other (Real)')")],
                'lines': [1, 0, 'number', _render("p['ps_other_real']"), None, self.pd_cell_style_decimal],
                'totals': [1, 22, 'number', _render("p['ps_other_real']"), None, self.rt_cell_style_decimal]},
            'ps_total_real': {
                'header': [1, 22, 'text', _render("_('PS Total (Real)')")],
                'lines': [1, 0, 'number', _render("p['ps_total_real']"), None, self.pd_cell_style_decimal],
                'totals': [1, 22, 'number', _render("p['ps_total_real']"), None, self.rt_cell_style_decimal]},
            'price_unit': {
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ws_o = wb.add_sheet('Laporan Jual Acc')
            ...
            c_specs_o = [('report_name', 1, 0, 'text', 'Laporan Jual Acc')]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(ws_o, row_pos_o, row_data, row_style=cell_style)
            ...
            ws_o.write(row_pos_o, 53, xlwt.Formula("SUM(BB"+str(row_data_begin)+":BB"+str(row_data_end)+")"), self.rt_cell_style_decimal)
            ws_o.write(row_pos_o, 54, xlwt.Formula("SUM(BC"+str(row_data_begin)+":BC"+str(row_data_end)+")"), self.rt_cell_style_decimal)
            ws_o.write(row_pos_o, 55, xlwt.Formula("SUM(BD"+str(row_data_begin)+":BD"+str(row_data_end)+")"), self.rt_cell_style_decimal)
            ws_o.write(row_pos_o, 56, xlwt.Formula("SUM(BE"+str(row_data_begin)+":BE"+str(row_data_end)+")"), self.rt_cell_style_decimal)
            ws_o.write(row_pos_o, 57, xlwt.Formula("SUM(BF"+str(row_data_begin)+":BF"+str(row_data_end)+")"), self.rt_cell_style_decimal)
...
report_rjualacc_xls('report.Laporan Jual Acc', 'dealer.spk', parser = dym_report_rjualacc_print_xls)
