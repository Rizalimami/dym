...
class dym_purchase_requisition(osv.osv):
    ...
    def wkf_request_approval(self, cr, uid, ids, context=None):    
        ...
        if obj_pr.division not in ('Unit','Sparepart'):
    ...
    def wkf_approval(self, cr, uid, ids, context=None):        
        ...
        if not obj_pr.line_ids:
            raise osv.except_osv(('Perhatian !'), ("Produk belum diisi"))
        if obj_pr.division in ('Unit','Sparepart'):
            self.write(cr, uid, ids, {'approval_state':'a'})
        else:
            approval_sts = self.pool.get("dym.approval.matrixbiaya").approve(cr, uid, ids, obj_pr)
            if approval_sts == 1:
                self.write(cr, uid, ids, {'approval_state':'a'})
            elif approval_sts == 0:
    ...
    def only_unit_or_sparepart(self, cr, uid, ids, *args):        
