...
class report_stock_unit_wizard(orm.TransientModel):
    _name = 'report.stock.unit.wizard'
    _description = 'Report Stock Unit Wizard'
 
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if not context: context = {}
        res = super(report_stock_unit_wizard, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = { 
        'branch_ids': fields.many2many('dym.branch', 'report_stock_unit_invoice_rel', 'report_stock_unit_wizard_id',
                                        'branch_id', 'Branch', copy=False),
        'product_ids': fields.many2many('product.product', 'report_stock_unit_product_rel', 'report_stock_unit_wizard_id',
                                        'product_id', 'Product', copy=False, ),
        'location_ids': fields.many2many('stock.location', 'report_stock_unit_location_rel', 'report_stock_unit_wizard_id',
    ...
    def create(self,cr,uid,vals,context=None):
        ...
        res = super(report_stock_unit_wizard,self).create(cr,uid,vals,context=context)
        return res   
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        if context.get('xls_export'):
            return {'type': 'ir.actions.report.xml',
                    'report_name': 'dym.stock.report.unit.xls',
                    'datas': data}
        else:
            context['landscape'] = True
            return self.pool['report'].get_action(
                cr, uid, [],
                'dym_stock.report_stock',
