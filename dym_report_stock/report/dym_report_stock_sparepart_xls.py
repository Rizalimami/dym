...
from .dym_report_stock_sparepart import dym_stock_sparepart_report_print
...
class dym_stock_report_sparepart_print_xls(dym_stock_sparepart_report_print):

    def __init__(self, cr, uid, name, context):
        super(dym_stock_report_sparepart_print_xls, self).__init__(
            cr, uid, name, context=context)
        quant_obj = self.pool.get('stock.quant')
        self.context = context
        wl_overview = quant_obj._report_xls_stock_sparepart_fields(
            cr, uid, context)
...
class stock_report_sprepart_xls(report_xls):
    ...
    def __init__(self, name, table, rml=False,
        ...
        self.col_specs_template_overview = {
            ...
            'branch_id': {
                'header': [1, 35, 'text', _render("_('Cabang ID')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['branch_id'] or 'n/a'")],
                'totals': [1, 0, 'text', None]},
            'branch_name': {
            ...
            'default_code': {
                ...
                'totals': [1, 0, 'text', None]},
            'category_id': {
                'header': [1, 22, 'text', _render("_('Categ ID')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['category_id'] or 'n/a'")],
                'totals': [1, 0, 'text', None]},
            'categ_name': {
            ...
            'product_name_tmpl_id': {
                'header': [1, 22, 'text', _render("_('Product Template ID')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['product_name_tmpl_id'] or 'n/a'")],
                'totals': [1, 0, 'text', None]},
            'product_name': {
                ...
                'totals': [1, 0, 'text', None]},
            'p_product_id': {
                'header': [1, 22, 'text', _render("_('Product ID')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['p_product_id'] or 'n/a'")],
                'totals': [1, 0, 'text', None]},
            'move_origin': {
            ...
            'nilai': {
                ...
                'lines': [1, 0, 'number', _render("p['harga_satuan']")],
            ...
            'location_name': {
                ...
                'totals': [1, 0, 'text', None]},
            'loc_id': {
                'header': [1, 35, 'text', _render("_('Lokasi ID')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['loc_id'] or 'n/a'")],
                'totals': [1, 0, 'text', None]},
            'status': {
                ...
                'totals': [1, 0, 'text', None]}, 
            'pack_name': {
                'header': [1, 15, 'text', _render("_('No GRN')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['pack_name']")],
                'totals': [1, 0, 'text', None]},           
            'pack_date': {
                'header': [1, 15, 'text', _render("_('Tgl GRN')"),None,self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['pack_date']")],
                'totals': [1, 0, 'text', None]},                     
            'ranking': {
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            report_name = ' '.join(
                [_p.company.name, r['title'], _('Laporan Analisa Mutasi Stock Sparepart'),
                 _p.report_info])
            c_specs_o = [
            ...
            ws_o.write(row_pos_o, 17, "Total",self.ph_cell_style) 
            ws_o.write(row_pos_o, 18, xlwt.Formula("SUM(N"+str(row_data_begin)+":N"+str(row_data_end)+")"),self.pd_cell_style_number_fill)
...
stock_report_sprepart_xls(
    'report.dym.stock.sparepart.report.xls',
    'account.period',
    parser=dym_stock_report_sparepart_print_xls)
