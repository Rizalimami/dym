...
from .dym_report_penjualansdb import dym_report_penjualansdb_print
from openerp.tools.translate import translate
import string

_ir_translation_name = 'report.penjualansdb'

class dym_report_penjualansdb_print_xls(dym_report_penjualansdb_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_penjualansdb_print_xls, self).__init__(cr, uid, name, context=context)
        dso_obj = self.pool.get('dealer.sale.order')
        self.context = context
        wl_overview = dso_obj._report_xls_penjualansdb_fields(cr, uid, context)
...
class report_penjualansdb_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_penjualansdb_xls, self).__init__(name, table, rml, parser, header, store)
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            c_specs_o = [
                ('report_name', 1, 0, 'text', 'Laporan SDB New'),
            ]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(
                ws_o, row_pos_o, row_data, row_style=cell_style)
            ...
            ws_o.write(row_pos_o, 61, xlwt.Formula("SUM(BJ"+str(row_data_begin)+":BJ"+str(row_data_end)+")"), self.rt_cell_style_decimal)
            ...
            ws_o.write(row_pos_o, 77, xlwt.Formula("SUM(BZ"+str(row_data_begin)+":BZ"+str(row_data_end)+")"), self.rt_cell_style_decimal)
            ...
            ws_o.write(row_pos_o, 99, None, self.rt_cell_style_decimal)
...
report_penjualansdb_xls(
    'report.Laporan SDB New',
    'dealer.sale.order',
    parser = dym_report_penjualansdb_print_xls)
