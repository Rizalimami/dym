...
class account_voucher(models.Model):
    ...
    @api.one
    @api.depends('journal_id','net_amount')
    def _journal_is_bank(self):
        self.journal_is_bank = True if self.journal_id.type == 'bank' and ((self.net_amount > 0 and self.type == 'payment') or (self.amount < 0 and self.type == 'receipt')) else False

    @api.onchange('journal_id','net_amount')
    def payment_method_change(self):
        self.clearing_bank = False
        if self.journal_id.type == 'bank' and ((self.net_amount > 0 and self.type == 'payment') or (self.net_amount < 0 and self.type == 'receipt')):
    ...
    def _get_amount(self, inverse=False):
        if inverse == False:
    ...
    def create_first_line(
        ...
        lines = self.prepare_move_line(
                voucher, amount, move_id, name, company_currency,
                current_currency, payment_date, account, partner)
        if voucher.clearing_bank == True:
            lines['account_id'] = voucher.journal_id.get_clearing_account_id()
            lines['clear_state'] = 'open'
        move_line = move_lines.create(lines)
        return move_line
    ...
    def prepare_move_line(
        ...
        move_line = {
            ...
            'date': voucher.value_date,
            'date_maturity': date_due or False,
            'analytic_account_id': voucher.analytic_4.id,
