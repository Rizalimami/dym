...
class dym_report_penjualan_so_wizard(orm.TransientModel):
    _name = 'dym.report.penjualan.so.wizard'
    _description = 'Report Penjualan SO Wizard'
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        res = super(dym_report_penjualan_so_wizard, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = {
        'category': fields.selection([('Sparepart','Sparepart'), ('ACCESSORIES','Aksesoris')], 'Category', change_default=True, select=True),
        'section_id': fields.many2one('crm.case.section', 'Sales Team'),
        'user_id': fields.many2one('hr.employee', 'Sales Person'),
        'product_ids': fields.many2many('product.product', 'dym_report_penjualan_so_product_rel', 'dym_report_penjualan_so_wizard_id',
            'product_id', 'Products'),
        ...
        'branch_ids': fields.many2many('dym.branch', 'dym_report_penjualan_so_branch_rel', 'dym_report_penjualan_so_wizard_id',
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        if context.get('xls_export') :
            return {'type': 'ir.actions.report.xml',
                    'report_name': 'Laporan Penjualan SO',
                    'datas': data}
        else :
            context['landscape'] = True
            return self.pool['report'].get_action(cr, uid, [], 'dym_report_penjualan_so.report_penjualan_so', data=data, context=context)
