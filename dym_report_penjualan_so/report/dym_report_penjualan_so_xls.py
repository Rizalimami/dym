...
from .dym_report_penjualan_so import dym_report_penjualan_so_print
from openerp.tools.translate import translate
import string

_ir_translation_name = 'report.penjualan.so'

class dym_report_penjualan_so_print_xls(dym_report_penjualan_so_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_penjualan_so_print_xls, self).__init__(cr, uid, name, context=context)
        so_obj = self.pool.get('sale.order')
        self.context = context
        wl_overview = so_obj._report_xls_penjualan_so_fields(cr, uid, context)
...
class report_penjualan_so_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_penjualan_so_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_overview = {
            ...
            'discount': {
                'header': [1, 22, 'text', _render("_('Discount2')")],
            ...
            'discount_show': {
                'header': [1, 22, 'text', _render("_('Discount1')")],
                'lines': [1, 0, 'number', _render("p['discount_show']"), None, self.pd_cell_style_decimal],
                'totals': [1, 22, 'number', _render("p['discount_show']"), None, self.rt_cell_style_decimal]},
            'discount_program': {
            ...
            'discount_cash': {
                'header': [1, 22, 'text', _render("_('Discount Cash')")],
                'lines': [1, 0, 'number', _render("p['discount_cash']"), None, self.pd_cell_style_decimal],
                'totals': [1, 22, 'number', _render("p['discount_cash']"), None, self.rt_cell_style_decimal]},
            'discount_lain': {
                'header': [1, 22, 'text', _render("_('Discount Lain')")],
                'lines': [1, 0, 'number', _render("p['discount_lain']"), None, self.pd_cell_style_decimal],
                'totals': [1, 22, 'number', _render("p['discount_lain']"), None, self.rt_cell_style_decimal]},
            'price_subtotal': {
            ...
            'invoice_status': {
                'header': [1, 22, 'text', _render("_('Invoice Status')")],
                'lines': [1, 0, 'text', _render("p['invoice_status']")],
                'totals': [1, 22, 'number', None]},
            'program_subsidi': {
                'header': [1, 22, 'text', _render("_('Program Subsidi')")],
                'lines': [1, 0, 'text', _render("p['program_subsidi']")],
                'totals': [1, 22, 'number', None]},
        }
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            c_specs_o = [
                ('report_name', 1, 0, 'text', 'LAPORAN PENJUALAN SO'),
            ]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(
                ws_o, row_pos_o, row_data, row_style=cell_style)
...
report_penjualan_so_xls(
    'report.Laporan Penjualan SO',
    'sale.order',
    parser = dym_report_penjualan_so_print_xls)
