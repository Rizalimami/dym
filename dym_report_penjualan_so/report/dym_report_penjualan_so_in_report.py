...
class dym_report_penjualan_so_report(osv.osv):
    _name = "dym.report.penjualan.so.report"
    _description = "Report Penjualan SO"
    _auto = False
    _columns = {
        ...
        'user_id': fields.many2one('hr.employee', 'Salesperson', readonly=True),
    ...
    def init(self, cr):
        tools.sql.drop_view_if_exists(cr, 'dym_report_penjualan_so_report')
        cr.execute("""
            create or replace view dym_report_penjualan_so_report as (
