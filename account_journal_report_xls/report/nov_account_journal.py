...
class nov_journal_print(report_sxw.rml_parse):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        #b_obj = self.pool.get('dym.branch')
        ...
        if self.print_by == 'period':
            ...
            for jp in journal_period_ids:
                ...
                #branch = b_obj.browse(self.cr, self.uid, jp[2], self.context)
                #print jp ,'mmmmmmmm', journal_period_ids,'mmmmm', journal,'mmmmmmmmm',periods,'mmmmmmm',branch_ids


                objects.extend([(journal,period,branch_ids) for period in periods])
            ...
            for jf in journal_fy_ids:
                ...
                objects.append((journal,fiscalyear,branch_ids))
                self.localcontext['objects'] = self.objects = objects
    ...
    def _lines(self, object):
        ...
        # print str(tuple(period_ids)).replace(',)', ')')
        # print journal_id, ','
        # print str(tuple(self.move_states)).replace(',)', ')')

        if branch_ids:
            where_branch = "  %s " % branch_ids
            ...
            self.cr.execute("SELECT l.move_id AS move_id, l.id AS aml_id, "
                        ...
                        "LEFT JOIN dym_branch db "
                        "ON l.branch_id = db.id "
                        "WHERE l.period_id IN %s AND l.journal_id = %s "
                        "AND am.state IN %s " + where_extra +
                        " and l.branch_id in %s ORDER BY " + self.sort_selection +
                        ", move_date, move_id, acc_code",
                        (tuple(period_ids), journal_id,
                         tuple(self.move_states),tuple(branch_ids)))
        else:
            self.cr.execute("SELECT l.move_id AS move_id, l.id AS aml_id, "
                            "am.name AS move_name, "
                            "db.name as branch_name, "
