...
from .dym_report_penjualan_spbynopol import dym_report_penjualan_spbynopol_print
from openerp.tools.translate import translate
import string

_ir_translation_name = 'report.penjualan.spbynopol'

class dym_report_penjualan_spbynopol_print_xls(dym_report_penjualan_spbynopol_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_penjualan_spbynopol_print_xls, self).__init__(cr, uid, name, context=context)
        sp_obj = self.pool.get('dym.work.order')
        self.context = context
        wl_overview = sp_obj._report_xls_penjualan_wobynopol_fields(cr, uid, context)
...
class report_penjualan_spbynopol_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_penjualan_spbynopol_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_overview = {
            ...
            'no_pol': {
                'header': [1, 10, 'text', _render("_('No Polisi')"), None, self.rh_cell_style_center],
                'lines': [1, 0, 'text', _render("p['no_pol'] or 'n/a'")],
...
report_penjualan_spbynopol_xls(
    'report.Laporan Penjualan WO by NOPOL',
    'dym.work.order',
    parser = dym_report_penjualan_spbynopol_print_xls)
