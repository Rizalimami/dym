...
class dym_report_penjualantax_wizard(orm.TransientModel):
    _name = 'dym.report.penjualantax.wizard'
    _description = 'Report Penjualan Tax Wizard'
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        res = super(dym_report_penjualantax_wizard, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = {
        ...
        'product_ids': fields.many2many('product.product', 'dym_report_penjualantax_product_rel', 'dym_report_penjualantax_wizard_id',
            'product_id', 'Products'),
        ...
        'branch_ids': fields.many2many('dym.branch', 'dym_report_penjualantax_branch_rel', 'dym_report_penjualantax_wizard_id',
            'branch_id', 'Branches', copy=False),
        'finco_ids': fields.many2many('res.partner', 'dym_report_penjualantax_partner_rel', 'dym_report_penjualantax_wizard_id',
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        if context.get('xls_export') == 'detail_per_chassis_engine' :
            return {'type': 'ir.actions.report.xml',
                    'report_name': 'Laporan Penjualan Tax',
                    'datas': data}
        else :
            # bawah perintah report generate
            raise osv.except_osv(('Perhatian'), ('Report untuk options ini belum tersedia !'))
            context['landscape'] = True
            return self.pool['report'].get_action(cr, uid, [], 'dym_report_penjualantax.report_penjualantax', data=data, context=context)
