...
class dym_edc(models.Model):
    ...
    def first_move_line_get(self, cr, uid, voucher_id, move_id, company_currency, current_currency, context=None):
        move_line = super(dym_edc, self).first_move_line_get(cr,uid,voucher_id, move_id, company_currency, current_currency, context=context)
...
class dym_edc_journal(models.Model):
    _inherit = "account.journal"
                
    partner_id = fields.Many2one('res.partner',string="Partner") 
 
class dym_edc_journal_type(orm.Model):
    _inherit = "account.journal"
    ...
    def _register_hook(self, cr):      
        ...
        if ('edc','EDC') not in selection:
            self._columns['type'].selection.append(
                ('edc', 'EDC'))
        return super(dym_edc_journal_type, self)._register_hook(cr)                            
