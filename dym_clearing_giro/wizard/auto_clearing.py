...
class AutoClearing(models.TransientModel):
    ...
    def action_process(self):        
        ...
        for move_line in move_line_ids:
            ...
            if move_line.move_id.model == 'dym.bank.transfer' and move_line.move_id.transaction_id:
                trf_id = self.env['dym.bank.transfer'].browse([move_line.move_id.transaction_id])
                if trf_id.transaction_type == 'inhouse':
                    continue
                if trf_id.ibanking_id:
                    continue
