...
class Eksport_distribution(osv.osv_memory):
    ...
    def export_file(self, cr, uid, ids, context=None):
        ...
        trx_obj = self.pool.get('dym.stock.packing').browse(cr,uid,trx_id,context=context)
    ...
    def eksport_picking(self, cr, uid, ids,trx_obj, context=None):
        ...
        branch=self.pool.get('dym.branch').search(cr,uid,[('code','=','MML')])
        ...
        for y in obj_packing_browse :
            ...
            if y.model_id.model :
                ...
                if y.model_id.model == 'dym.mutation.order' :
