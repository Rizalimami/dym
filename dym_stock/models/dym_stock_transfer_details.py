...
class dym_stock_transfer_details(models.TransientModel):
    _name = 'stock.dym_transfer_details'
    _description = 'Picking wizard'

    picking_id = fields.Many2one('stock.picking', 'Picking')
    item_ids = fields.One2many('stock.dym_transfer_details_items', 'transfer_id', 'Items', domain=[('product_id', '!=', False)])
    packop_ids = fields.One2many('stock.dym_transfer_details_items', 'transfer_id', 'Packs', domain=[('product_id', '=', False)])
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        res = super(dym_stock_transfer_details, self).default_get(cr, uid, fields, context=context)
    ...
    def _get_default_date(self):
        return self.env['dym.branch'].get_default_date()
    ...
    def do_detailed_transfer(self) :
        ...
        try :
            ...
            if code == 'internal' and destloc_type == 'transit' :
                obj_mo_id = self.env['dym.mutation.order'].browse(self.picking_id.transaction_id)
    ...
    def wizard_view(self):
        view = self.env.ref('dym_stock.view_stock_enter_transfer_details')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'stock.dym_transfer_details',
            'views': [(view.id, 'form')],
            'view_id': view.id,
            'target': 'new',
            'res_id': self.ids[0],
...
class dym_stock_transfer_details_items(models.TransientModel):
    _name = 'stock.dym_transfer_details_items'
    ...
    transfer_id = fields.Many2one('stock.dym_transfer_details', 'Transfer')
    ...
    branch_id = fields.Many2one('dym.branch', 'Branch', default=_get_default_branch)
        ...
        if (code == 'incoming' and picking_id.division == 'Unit') or (code == 'internal' and picking_id.division == 'Unit' and destloc_type == 'transit') or (code == 'internal' and picking_id.division == 'Unit' and sourceloc_type == 'transit') :
            if self.engine_number and (not self.product_id or categ_id.isParentName('Unit')):
                ...
                if len(self.engine_number) != 12 :
                    ...
                    warning['message']=_('Silahkan periksa kembali Engine Number yang Anda input (1)')
