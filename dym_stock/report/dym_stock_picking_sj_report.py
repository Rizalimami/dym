...
class stock_picking(report_sxw.rml_parse):
    ...
    def mutation_order(self):
         ...
         mutation_order=self.pool.get(obj_picking.model_id).browse(self.cr,self.uid,obj_picking.transaction_id)
         return mutation_order
    
    def jumlah_cetakan(self):
        obj_model = self.pool.get('ir.model')
        obj_model_id = obj_model.search(self.cr, self.uid,[ ('model','=','stock.picking') ])[0]
        obj_ir = self.pool.get('ir.actions.report.xml').search(self.cr, self.uid,[('report_name','=','rml.dym.stock.picking.sj')])
    ...
    def no_invoice(self):
        obj_picking = self.pool.get('stock.picking').browse(self.cr, self.uid, self.ids).origin
        invoice_search = self.pool.get('account.invoice').search(self.cr, self.uid,[('origin','ilike',obj_picking),('tipe','=','customer')])
        if not invoice_search:
            invoice_search = self.pool.get('account.invoice').search(self.cr, self.uid,[('origin','ilike',obj_picking),('tipe','=','finco')])
        no_invoice = self.pool.get('account.invoice').browse(self.cr, self.uid,invoice_search)
        return no_invoice
    
report_sxw.report_sxw('report.rml.dym.stock.picking.sj', 'stock.picking', 'addons/dym_stock/report/dym_stock_picking_sj_report.rml', parser = stock_picking, header = False)