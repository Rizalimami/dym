...
class dym_stock_packing(report_sxw.rml_parse):
    ...
    def mutation_order(self):
        packing_id=self.pool.get('dym.stock.packing').browse(self.cr, self.uid, self.ids).picking_id.id
        ...
        elif obj_picking.origin[0:3] in ['DSM','SOR','WOR'] :
    ...
    def jumlah_cetakan(self):
        ...
        obj_model_id = obj_model.search(self.cr, self.uid,[ ('model','=','dym.stock.packing') ])[0]
        obj_ir = self.pool.get('ir.actions.report.xml').search(self.cr, self.uid,[('report_name','=','rml.dym.stock.packing.surat.jalan')])
...
report_sxw.report_sxw('report.rml.dym.stock.packing.surat.jalan', 'dym.stock.packing', 'addons/dym_stock/report/dym_stock_surat_jalan_report.rml', parser = dym_stock_packing, header = False)