...
from .dym_report import dym_report_distribusi_unit_print
...
class dym_report_distribusi_unit_print_xls(dym_report_distribusi_unit_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_distribusi_unit_print_xls, self).__init__(
            cr, uid, name, context=context)
        move_line_obj = self.pool.get('purchase.order')
        self.context = context
        wl_overview = move_line_obj._report_xls_distribusi_unit_fields(
            cr, uid, context)
...
class report_distribusi_unit_xls(report_xls):

    def __init__(self, name, table, rml=False,
                 parser=False, header=True, store=False):
        super(report_distribusi_unit_xls, self).__init__(
        ...
        self.col_specs_template_overview = {
            ...
            'tipe_product': {
                ...
                'totals': [1, 22, 'text', None]},         
            'internal_reference': {
                'header': [1, 22, 'text', _render("_('Produk Description')")],
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            report_name = ' '.join(
                [_('LAPORAN Distribusi Unit Per Tanggal'), _(str(datetime.today().date())),
                 _p.report_info])
            c_specs_o = [
                ('report_name', 1, 0, 'text', report_name),
            ]
...
report_distribusi_unit_xls('report.Laporan Distribusi Unit', 'purchase.order', parser = dym_report_distribusi_unit_print_xls)
