...
class dym_report_penjualanacc_print(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        if context is None:
            context = {}
        super(dym_report_penjualanacc_print, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        report_penjualanacc = {
            'type': '',
            'title': '',
            'title_short': title_short_prefix + ', ' + _('Laporan Penjualan Acc')}
        ...
        reports = [report_penjualanacc]
        for report in reports:
            cr.execute(query_penjualanacc)
            # print query_penjualanacc
            ...
            if all_lines:
                ...
                p_map = map(
                    lambda x: {
                        ...
                        'cust_name_stnk': str(x['cust_name'].encode('ascii','ignore').decode('ascii')) if x['cust_name'] != None else '',
                    ...
                    for z in y['lines']:
                        if z['lot_name'] != temp_str:
                            ...
                            test_tampung = {
                                ...
                                'ps_dealer_real' : temp['ps_dealer_real'],
        ...
        if not reports :
            ...
            'faktur_pajak': 'NO DATA FOUND'}], 'title_short': 'Laporan Penjualan Acc', 'type': '', 'title': ''}]
        ...
        super(dym_report_penjualanacc_print, self).set_context(
    ...
    def formatLang_zero2blank(self, value, digits=None, date=False, date_time=False, grouping=True, monetary=False, dp=False, currency_obj=False):
        if isinstance(value, (float, int)) and not value:
            ...
            return super(dym_report_penjualanacc_print, self).formatLang(value, digits, date, date_time, grouping, monetary, dp, currency_obj)

#register report
class wrapped_vat_declaration_print(orm.AbstractModel):
    _name = 'report.dym_report_penjualanacc.report_penjualanacc'
    _inherit = 'report.abstract_report'
    _template = 'dym_report_penjualanacc.report_penjualanacc'
    _wrapped_report_class = dym_report_penjualanacc_print
