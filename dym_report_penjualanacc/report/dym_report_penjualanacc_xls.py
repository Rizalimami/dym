...
from .dym_report_penjualanacc import dym_report_penjualanacc_print
from openerp.tools.translate import translate
import string

_ir_translation_name = 'report.penjualanacc'

class dym_report_penjualanacc_print_xls(dym_report_penjualanacc_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_penjualanacc_print_xls, self).__init__(cr, uid, name, context=context)
        dso_obj = self.pool.get('dealer.sale.order')
        self.context = context
        wl_overview = dso_obj._report_xls_penjualanacc_fields(cr, uid, context)
...
class report_penjualanacc_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_penjualanacc_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_overview = {
            ...
            'invoice_date': {
                ...
                'totals': [1, 22, 'number', None]},
            'effective_date': {
                'header': [1, 22, 'text', _render("_('Date Effective')")],
                'lines': [1, 0, 'text', _render("p['effective_date']")],
                'totals': [1, 22, 'number', None]},
            'tgl_lunas': {
            ...
            'cust_name_stnk': {
                ...
                'lines': [1, 0, 'text', _render("p['cust_name_stnk']")],
                'totals': [1, 22, 'number', None]},
            'product_name': {
            ...
            'tgl_po': {
                    ...
                    'totals': [1, 22, 'number', None]},
            'ps_dealer_real': {
                'header': [1, 22, 'text', _render("_('DPP PS Dealer (Real)')")],
            ...
            'ps_ahm_real': {
                'header': [1, 22, 'text', _render("_('DPP PS AHM (Real)')")],
            ...
            'ps_md_real': {
                'header': [1, 22, 'text', _render("_('DPP PS MD (Real)')")],
            ...
            'ps_finco_real': {
                'header': [1, 22, 'text', _render("_('DPP PS Finco (Real)')")],
            ...
            'ps_total_real': {
                'header': [1, 22, 'text', _render("_('DPP PS Total (Real)')")],
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            c_specs_o = [
                ('report_name', 1, 0, 'text', 'LAPORAN PENJUALAN ACC'),
            ]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(
                ws_o, row_pos_o, row_data, row_style=cell_style)
...
report_penjualanacc_xls(
    'report.Laporan Penjualan Acc',
    'dealer.sale.order',
    parser = dym_report_penjualanacc_print_xls)
