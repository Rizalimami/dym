...
class dym_report_penjualan_sp_print(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        if context is None:
            context = {}
        super(dym_report_penjualan_sp_print, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        query_penjualan_sp = """select sp.id as id_sp, 
                        ...
                        if not wo_line.bundle_line:
                            ...
                            if wo_line and wo_line.product_id.categ_id:
                                ...
                                if inv_id:
                                    ...
                                    if inv.payment_ids:
                                        ...
                                        p.update({'ar': ar})
                                    inv_id_line = self.pool.get('account.invoice.line').search(cr, uid, [('invoice_id', '=', inv.id),('force_cogs', '>', 0) ], limit=1)
                                    #print '1ccccc',inv_id_line
                                    if inv_id_line:
                                        inv_line = self.pool.get('account.invoice.line').browse(cr, uid, inv_id_line)
                                        #print 'xxx',inv_line.force_cogs
                                        if inv_line.force_cogs >0:
                                            p.update({'force_cogs2': inv_line.force_cogs})
                                        if  p['supply_qty2'] > 0:
                                            p.update({'supply_qty': p['supply_qty2']})


                                p.update({'invoice_name': invoice_name})
                                p.update({'invoice_date': invoice_date})

                                #print 'zzzzzz', p['force_cogs']
                                if p['force_cogs'] == None:
                                    p.update({'force_cogs': p['force_cogs2']})
                            ...
                            for line in wo_line.bundle_line:
                                dicts = {
                                    ...
                                    'name': p['name'],
                                    'oos_number': p['oos_number'],
                                    'oos_tgl': p['oos_tgl'],
                                    'dno_number': p['dno_number'],
                                    'dno_tgl': p['dno_tgl'],
                                    ...
                                    'dpp': ((line.product_uom_qty * line.price_bundle) - line.diskon)/1.1,
                                    'ppn': (((line.product_uom_qty * line.price_bundle) - line.diskon)/1.1) * 0.1,
                                    'force_cogs': p['force_cogs'],
                                    ...
                                    'force_cogs2': p['force_cogs2'],
                                    'supply_qty2': p['supply_qty2'],
                                ...
                                if line and line.product_id.categ_id:
                                    ...
                                    if inv_id:
                                        ...
                                        if inv.payment_ids:
                                            ...
                                            dicts.update({'ar': ar})
                                        inv_id_line = self.pool.get('account.invoice.line').search(cr, uid, [('invoice_id', '=', inv.id), ('force_cogs', '>', 0)], limit=1)
                                        #print '2ccccc', inv_id_line
                                        if inv_id_line:
                                            inv_line = self.pool.get('account.invoice.line').browse(cr, uid,inv_id_line)
                                            #print 'yyyyyyyy',p['force_cogs']
                                            if p['force_cogs'] == None:
                                                #print inv_line.force_cogs,'tttttt',p['supply_qty2']
                                                dicts.update({'force_cogs': inv_line.force_cogs})
                                                dicts.update({'supply_qty': p['supply_qty2']})
                            ...
                            res[0].update({'dpp': (res[0]['price_subtotal'])*1.1 -  p['discount_program'] - p['discount']})
                            res[0].update({'ppn': ((res[0]['price_subtotal'])*1.1)*0.1 -  p['discount_program'] - p['discount']})
                            #print 'xxxx', p['force_cogs'],p['force_cogs2']
                            #if p['force_cogs'] == None:
                            #    print 'cccc', p['force_cogs'], p['force_cogs2']
                            #    res[0].update({'force_cogs': p['force_cogs2']})
                            sp_ids += res
        ...
        if not reports :
            ...
            'faktur_pajak': 'NO DATA FOUND'}], 'title_short': 'Laporan Penjualan WO', 'type': '', 'title': ''}]
        ...
        super(dym_report_penjualan_sp_print, self).set_context(
    ...
    def formatLang_zero2blank(self, value, digits=None, date=False, date_time=False, grouping=True, monetary=False, dp=False, currency_obj=False):
        if isinstance(value, (float, int)) and not value:
            ...
            return super(dym_report_penjualan_sp_print, self).formatLang(value, digits, date, date_time, grouping, monetary, dp, currency_obj)

class wrapped_vat_declaration_print(orm.AbstractModel):
    _name = 'report.dym_report_penjualan_sp.report_penjualan_sp'
    _inherit = 'report.abstract_report'
    _template = 'dym_report_penjualan_sp.report_penjualan_sp'
    _wrapped_report_class = dym_report_penjualan_sp_print
