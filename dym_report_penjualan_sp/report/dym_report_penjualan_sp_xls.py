...
from .dym_report_penjualan_sp import dym_report_penjualan_sp_print
from openerp.tools.translate import translate
import string

_ir_translation_name = 'report.penjualan.sp'

class dym_report_penjualan_sp_print_xls(dym_report_penjualan_sp_print):

    def __init__(self, cr, uid, name, context):
        super(dym_report_penjualan_sp_print_xls, self).__init__(cr, uid, name, context=context)
        sp_obj = self.pool.get('dym.work.order')
        self.context = context
        wl_overview = sp_obj._report_xls_penjualan_wo_fields(cr, uid, context)
...
class report_penjualan_sp_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_penjualan_sp_xls, self).__init__(name, table, rml, parser, header, store)
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ...
            c_specs_o = [
                ('report_name', 1, 0, 'text', 'LAPORAN PENJUALAN WO'),
            ]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(
                ws_o, row_pos_o, row_data, row_style=cell_style)
...
report_penjualan_sp_xls(
    'report.Laporan Penjualan WO',
    'dym.work.order',
    parser = dym_report_penjualan_sp_print_xls)
