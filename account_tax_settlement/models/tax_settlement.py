    ...
    def prepare_line_values(self):
        ...
        for line in grouped_lines:
            ...
            vals = {
                ...
                'analytic_account_id' : self.tax_settlement_id.analytic_4.id,
                'branch_id' : self.tax_settlement_id.branch_id.id,
                'division' : self.tax_settlement_id.division,
            }
            res.append(vals)
        return res
...
class account_tax_settlement(models.Model):
    ...
    def _get_analytic_company(self):
        ...
        if not level_1_ids:
            raise osv.except_osv(('Perhatian !'), ("[account_tax_settlement] Tidak ditemukan data analytic untuk company %s")%(company.name))
    ...
    division = fields.Selection(DIVISION_SELECTION, 'Division', required=True)
